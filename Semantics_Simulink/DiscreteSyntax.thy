...
section \<open>Syntax for discrete diagram\<close>
...
\<comment> \<open>End syntax for discrete diagram\<close>

section \<open>well-formed definition for discrete diagram and some theorems for well-formed\<close>
...
lemma perm_notin: "Unique xs \<Longrightarrow> Unique ys \<Longrightarrow>
\<not> (xs <~~> ys) \<Longrightarrow> length xs = length ys \<Longrightarrow> (\<exists>j < length ys. (ys ! j \<notin> set xs \<and> 
(\<forall>i<j. ys ! i \<in> set xs)))"
...
next
  ...
  proof(cases "a \<in> set xs")
    case True
    have 0: "Unique (remove1 a xs)"
      by (simp add: Cons.prems(1) True Unique_remove)
    have 1: "\<not> (remove1 a xs) <~~> ys"
      using Cons(4) True by (meson perm_remove2 perm_sym)
    have 2: "\<exists>j<length ys. ys ! j \<notin> set (remove1 a xs) \<and> (\<forall>i<j. ys ! i \<in> set (remove1 a xs))"
      using 0 1 Cons by (metis True Unique_remove length_remove1 list.set_intros(1) remove_hd)
    obtain k where 3: "k<length ys \<and> ys ! k \<notin> set (remove1 a xs) \<and> (\<forall>i<k. ys ! i \<in> set (remove1 a xs))"
      using 2 by auto
    have 4: "ys ! k \<noteq> a"
      using Cons(3) unfolding Unique_def by (metis "3" Cons.prems(2) Unique_k gr_implies_not_zero 
          linorder_not_less list.set_intros(1) remove_hd)
    have 5: "k+1 <length (a # ys) \<and> (a # ys) ! (k+1) \<notin> set xs \<and> (\<forall>i<k+1. (a # ys) ! i \<in> set xs)"
      using 3 4 True by (metis Suc_eq_plus1 in_set_remove1 length_Cons less_Suc_eq_0_disj 
...
lemma Inputs_base2: "a \<in> set bl \<Longrightarrow>
Inputs (remove1 a bl) \<union> set (get_inputs a) = Inputs bl"
proof(induction bl)
  case Nil
  then show ?case by auto
next
  ...
  next
    case False
    have 1: "Inputs (remove1 a l) \<union> set (get_inputs a) = Inputs l"
...
lemma Inputs_perm: "al <~~> bl \<Longrightarrow> Inputs al = Inputs bl"
proof(induction al arbitrary:bl)
  case Nil
  then show ?case by auto
next
  ...
  then show ?case using Cons(1)[of "remove1 a bl"] 1 
    using Inputs.simps(2) Inputs_base2 by blast
qed
...
lemma InputsOutputs_forall: "\<forall>x\<in>set bl. set (get_outputs x) \<inter> Inputs cl = {} \<Longrightarrow>
Outputs bl \<inter> Inputs cl = {}"
proof(induction cl)
  case Nil
  then show ?case by simp
next
  case (Cons c cl)
  have 1: "Outputs bl \<inter> Inputs cl = {}"
    using Cons by auto
  have 2: "Outputs bl \<inter> set (get_inputs c) = {}"
  proof -
    have tmp1: "\<forall>x\<in>set bl. set (get_outputs x) \<inter> set (get_inputs c) = {}"
    ...
    next
      case (Cons b bl)
      then show ?case by (simp add: disjoint_iff)
    qed
  qed
...
lemma Graph_lemma3: "Unique bl \<Longrightarrow> a \<in> set bl \<Longrightarrow>
Graph (set bl) \<Longrightarrow> \<forall>x \<in> (Outputs (remove1 a bl)). x \<notin> set (get_outputs a)"
proof(induction bl)
  case Nil
  then show ?case by auto
next
  ...
  proof(cases "a = b")
    case True
    then show ?thesis using Cons(2,3,4) Graph_lemma2
      by (metis remove_hd)
  next
    case False
    have 1: "Unique bl"
      using Cons(2) Unique_lemma by fastforce
    have 2: "a \<in> set bl"
      using Cons(3) False by auto
    have 3: "Graph (set bl)"
      using Cons(4) Graph_lemma by simp
    have 4: "\<forall>x\<in>Outputs (remove1 a bl). x \<notin> set (get_outputs a)"
      using Cons(1) 1 2 3 by simp
    have 5: "remove1 a (b # bl) = b#(remove1 a bl)"
      using False by auto
    show ?thesis apply clarify subgoal premises pre for x
    proof(cases  "x \<in> set (get_outputs b)")
      case True
      then show ?thesis using pre(2) False Cons "2" Graph_lemma2 Outputs_base3 by blast
    next
      case False
      then show ?thesis using 4 pre 5 by simp
    qed
    done
qed 
qed
...
section \<open>functions and theorems for sort a diagram)\<close>
...
(*give a block list, return True if and only if it is sorted*)
...
lemma besorted_forall: "besorted bl \<Longrightarrow> (\<forall> i j. i < j \<and> j < length bl \<longrightarrow> 
            (check_offset (get_outputs (bl ! j)) (get_inputs (bl ! i)) (get_offsets (bl ! i))) = False)"
  apply clarify subgoal premises pre for i j using pre
  proof(induction bl arbitrary: i j)
    case Nil
    then show ?case by simp
  next
    case (Cons b bl)
    ...
    proof(cases "i = 0")
      case True
      then show ?thesis using Cons(2,3,4) unfolding besorted.simps by force
    next
      case False
      then show ?thesis using Cons(1)[of "i-1" "j-1"] Cons(2,3,4) unfolding besorted.simps by force
    qed
  qed
  done
...
lemma get_block_indegree_subset: "set bl \<subseteq> set bl' \<Longrightarrow> get_block_indegree b bl \<subseteq> get_block_indegree b bl'"
  unfolding get_block_indegree_def using Outputs_perm
  by (smt (z3) Int_iff Outputs_Cons Outputs_eq Un_iff set_append subsetI sup.orderE)
...
lemma find_0indegree_block_None : "fst (find_0indegree_blocks al bl cl) = None \<Longrightarrow>
  \<forall>b \<in> set bl. get_block_indegree b al -  (Outputs cl) \<noteq> {}"
  apply clarify subgoal premises pre for b using pre
  proof(induction bl)
    case Nil
    then show ?case by auto
  next
    ...
    then show ?case unfolding find_0indegree_blocks.simps 
      by (smt (verit, best) fst_conv option.simps(3) set_ConsD)
  qed
  done

lemma find_0indegree_block_None2 : "\<forall>b \<in> set bl. get_block_indegree b al -  (Outputs cl) \<noteq> {}
   \<Longrightarrow> fst (find_0indegree_blocks al bl cl) = None"
  proof(induction bl)
    case Nil
    then show ?case by auto
  next
    case (Cons b1 bl)
    then show ?case unfolding find_0indegree_blocks.simps by simp
  qed

lemma find_0indegree_block_perm : "fst (find_0indegree_blocks al1 bl1 cl1) = None \<Longrightarrow>
  bl1 <~~> bl2 \<Longrightarrow> cl1 <~~> cl2 \<Longrightarrow> al1 <~~> al2 \<Longrightarrow> fst (find_0indegree_blocks al2 bl2 cl2) = None"
  subgoal premises pre
proof -
  have 1: "\<forall>b \<in> set bl1. get_block_indegree b al1 -  (Outputs cl1) \<noteq> {}"
    using pre(1) find_0indegree_block_None by simp
  have 2: "\<forall>b \<in> set bl2. get_block_indegree b al2 -  (Outputs cl2) \<noteq> {}"
    using pre(2,3,4) 1 unfolding get_block_indegree_def by (metis Outputs_perm perm_sym prem_seteq)
  then show ?thesis using find_0indegree_block_None2 by simp
qed
  done
...
\<comment> \<open>sort algorithm find one 0-in-degree block at each cycle\<close>
...
\<comment> \<open>the following function is not used\<close>
fun topological_sort_kstep:: "block list \<Rightarrow> block list \<Rightarrow> sorted_block_list 
  \<Rightarrow> nat \<Rightarrow> block list \<times> sorted_block_list " where
  "topological_sort_kstep al bl cl 0 = (bl,cl)" |
  "topological_sort_kstep al bl cl k = (let x = (find_0indegree_blocks al bl cl) in 
                           (if (fst x = None) then (bl,cl) else
                                (topological_sort_kstep al (snd x) (cl@[the (fst x)]) (k-1))))"
...
\<comment> \<open>the following definitions and lemmas are proofs for topo-sort\<close>
...
lemma sort_subset2: "set (topological_sort al bl cl) \<subseteq> set (bl@cl)"
proof(induction bl arbitrary: cl rule: wf_induct[of "{(x, y). length x < length y}"])
  case 1
  then show ?case unfolding wf_def
    by (metis (no_types, lifting) case_prodD length_induct mem_Collect_eq)
next
  case (2 bl)
  then show ?case
  proof(cases "fst (find_0indegree_blocks al bl cl) = None")
    case True
    then show ?thesis by simp
  next
    case False
    ...
    have 4: "topological_sort al bl cl = (let x = find_0indegree_blocks al bl cl in 
      topological_sort al (snd x) (cl @ [the (fst x)]))"
      using topological_sort.simps[of al bl cl] 2 False
      by (smt (verit) append.left_neutral append_eq_append_conv)
    have 5: "set (bl) = set (let x = find_0indegree_blocks al bl cl in 
      ((snd x)@([the (fst x)])))"
      using False find_0indegree_lemma3 by (metis append_Cons append_Nil inf_sup_aci(5) set_append)
    have 6: "set (bl @ cl) = set (let x = find_0indegree_blocks al bl cl in 
      ((snd x)@(cl@[the (fst x)])))"
      using 5 by (metis append_assoc inf_sup_aci(5) set_append)
    then show ?thesis using 2(1) 3 6 4 by (metis case_prodI mem_Collect_eq)
  qed
qed
...
lemma toposort_Unique: "Unique(bl@cl) \<Longrightarrow> Unique (topological_sort al bl cl)"
proof(induction bl arbitrary: cl rule: wf_induct[of "{(x, y). length x < length y}"])
  case 1
  then show ?case unfolding wf_def
    by (metis (no_types, lifting) case_prodD length_induct mem_Collect_eq)
next
  ...
  proof(cases "fst (find_0indegree_blocks al bl cl) = None")
    ...
    then show ?thesis using Unique_Cons[of bl cl] 2(2) by simp
  next
    case False
    have 3: "length (snd (find_0indegree_blocks al bl cl)) < length bl"
      using 2 unfolding loop_free_def sortDiag_def using topological_sort.simps[of al bl cl]
    ...
    have 4: "topological_sort al bl cl = (let x = find_0indegree_blocks al bl cl in 
      ...
      using topological_sort.simps[of al bl cl] 2 False
      by (smt (verit) append.left_neutral append_eq_append_conv)
    have 5: "(bl@cl) <~~> (let x = find_0indegree_blocks al bl cl in 
      ((snd x)@(cl @ [the (fst x)])))"
      using find_0indegree_block_lemma2 find_0indegree_lemma4 False perm_remove3  by (smt (verit, 
        del_insts) find_0indegree_lemma3 list.set_intros(1) option.collapse prod.exhaust_sel)
    then show ?thesis using 2 3 4 Unique_perm by (metis case_prodI mem_Collect_eq)
  qed
qed

lemma toposort_perm2: "al1 <~~> al2 \<Longrightarrow> bl1@cl1 <~~> bl2@cl2 \<Longrightarrow> length cl1 = length cl2 \<Longrightarrow>
Unique (bl1@cl1) \<Longrightarrow> \<forall>i < length cl2. get_block_indegree (cl2 ! i) al2 -  (Outputs (take i cl2)) = {}
\<Longrightarrow> \<forall>i < length cl1. get_block_indegree (cl1 ! i) al1 -  (Outputs (take i cl1)) = {}
\<Longrightarrow> length (topological_sort al1 bl1 cl1) = length (topological_sort al2 bl2 cl2)"
proof(induction bl1 arbitrary: cl1 bl2 cl2 rule: wf_induct[of "{(x, y). length x < length y}"])
  case 1
  then show ?case unfolding wf_def
    by (metis (no_types, lifting) case_prodD length_induct mem_Collect_eq)
next
  case (2 bl1)
  then show ?case
  proof(cases "fst (find_0indegree_blocks al1 bl1 cl1) = None")
    case True
    note T1 = True
    have 3: "topological_sort al1 bl1 cl1 = cl1"
      unfolding topological_sort.simps[of al1 bl1 cl1]  using True by simp
    then show ?thesis
    proof(cases "cl1 <~~> cl2")
      case True
      have 4: "bl1 <~~> bl2"
        using 2(3) True by (meson perm.trans perm_append1 perm_append2_eq perm_sym)
      then show ?thesis using find_0indegree_block_perm 2(2,3,4) T1 True by simp
    next
      case False
      have 5: "Unique cl1 \<and> Unique cl2"
        using 2(3,5) Unique_Cons Unique_Permutation by blast
      obtain i where 6: "i < length cl2 \<and> (\<forall>j < i. cl2 ! j \<in> set cl1) \<and>  cl2 ! i \<notin> set cl1"
        using perm_notin[of cl1 cl2] 5 False 2(4) by blast
      have 7: "cl2 ! i \<in> set bl1"
        using 2(3) 6 by (smt (verit, ccfv_SIG) add_less_cancel_left add_less_cancel_right 
            length_append length_removeAll_less nth_append_length_plus nth_mem perm_sym 
            prem_seteq removeAll_append removeAll_id)
      have 8: "get_block_indegree (cl2 ! i) al1 -  (Outputs cl1) = {}"
      proof -
        have tmp1: "get_block_indegree (cl2 ! i) al2 -  (Outputs (take i cl2)) = {}"
          using 2(6) 6 by auto
        have tmp2: "\<forall>x \<in> set (take i cl2). x \<in> set cl1"
          using 6 by (smt (verit, best) append_self_conv append_take_drop_id diff_is_0_eq 
        in_set_conv_nth linorder_not_le nat_less_le nth_take take_all_iff take_append take_eq_Nil)
        have tmp3: "(Outputs (take i cl2)) \<subseteq> Outputs cl1"
          using tmp2 by (metis Outputs_Cons Outputs_eq Un_iff set_append subsetI sup.orderE)
        show ?thesis using 2(2) tmp3 get_block_indegree_perm tmp1 by force
      qed
      then show ?thesis using 7 T1 find_0indegree_block_None by blast
    qed
  next
    case False
    note F1 = False
    have 3: "length (snd (find_0indegree_blocks al1 bl1 cl1)) < length bl1"
      using 2 unfolding loop_free_def sortDiag_def using topological_sort.simps[of al1 bl1 cl1]
      by (smt (verit) False append_Nil append_eq_append_conv length_find_0indegree)
    have 4: "(bl1@cl1) <~~> (let x = find_0indegree_blocks al1 bl1 cl1 in 
      ((snd x)@(cl1 @ [the (fst x)])))"
      using find_0indegree_block_lemma2 find_0indegree_lemma4 False perm_remove3  by (smt (verit, 
        del_insts) find_0indegree_lemma3 list.set_intros(1) option.collapse prod.exhaust_sel)
    have 5: "Unique ((snd (find_0indegree_blocks al1 bl1 cl1))@(cl1 @ [the (fst (find_0indegree_blocks al1 bl1 cl1))]))"
      using 2(5) 4 Unique_perm by metis
    have 6: "fst (find_0indegree_blocks al2 bl2 cl2) \<noteq> None"
    proof(cases "cl1 <~~> cl2")
      case True
      have tmp1: "get_block_indegree (the (fst (find_0indegree_blocks al1 bl1 cl1))) 
          al1 - (Outputs cl1) = {}"
        using find_0indegree_block_lemma False by (metis eq_fst_iff option.collapse)
      have tmp2: "(the (fst (find_0indegree_blocks al1 bl1 cl1))) \<in> set bl2"
        using 2(3) True by (smt (verit, best) False find_0indegree_lemma3 list.set_intros(1) 
            perm.trans perm_append1 perm_append2_eq perm_sym prem_seteq)
      have tmp3: "get_block_indegree (the (fst (find_0indegree_blocks al1 bl1 cl1))) 
          al2 - (Outputs cl2) = {}"
        using True 2(2) Outputs_perm get_block_indegree_perm tmp1 by presburger
      then show ?thesis using tmp2 tmp3 find_0indegree_block_None by blast
    next
      case False
      have tmp1: "Unique cl1 \<and> Unique cl2"
        using 2(3,5) Unique_Cons Unique_Permutation by blast
      obtain i where tmp2: "i < length cl1 \<and> (\<forall>j < i. cl1 ! j \<in> set cl2) \<and>  cl1 ! i \<notin> set cl2"
        using perm_notin[of cl2 cl1] tmp1 False 2(4) perm_sym by auto
      have tmp3: "cl1 ! i \<in> set bl2"
        using 2(3) tmp2 by (smt (verit, ccfv_SIG) add_less_cancel_left add_less_cancel_right 
            length_append length_removeAll_less nth_append_length_plus nth_mem perm_sym 
            prem_seteq removeAll_append removeAll_id)
      have tmp4: "get_block_indegree (cl1 ! i) al2 -  (Outputs cl2) = {}"
      proof -
        have tmp4_1: "get_block_indegree (cl1 ! i) al1 -  (Outputs (take i cl1)) = {}"
          using 2(7) tmp2 by auto
        have tmp4_2: "\<forall>x \<in> set (take i cl1). x \<in> set cl2"
          using tmp2 by (smt (verit, best) append_self_conv append_take_drop_id diff_is_0_eq 
        in_set_conv_nth linorder_not_le nat_less_le nth_take take_all_iff take_append take_eq_Nil)
        have tmp4_3: "(Outputs (take i cl1)) \<subseteq> Outputs cl2"
          using tmp4_2 by (metis Outputs_Cons Outputs_eq Un_iff set_append subsetI sup.orderE)
        show ?thesis using 2(2) tmp4_3 get_block_indegree_perm tmp4_1 by force
      qed
      then show ?thesis using tmp3 find_0indegree_block_None by blast
    qed
    obtain b c where 7:"b = the (fst (find_0indegree_blocks al2 bl2 cl2)) \<and> c = 
        snd (find_0indegree_blocks al2 bl2 cl2) "
      using 6 by auto
    have 8: "length (cl1@[the (fst (find_0indegree_blocks al1 bl1 cl1))]) = 
        length (cl2@[b])"
      using 7 "2.prems"(3) by force
    have 9: "\<forall>i<length (cl2@[b]). get_block_indegree ((cl2@[b]) ! i) al2 
      - Outputs (take i (cl2@[b])) = {}"
      using 2(6) 6 by (smt (z3) "7" append_Nil2 diff_is_0_eq find_0indegree_block_lemma 
          le_eq_less_or_eq length_append_singleton less_Suc_eq nth_Cons_0 nth_append 
          option.collapse prod.exhaust_sel take_all_iff take_append take_eq_Nil)
    have 10: "\<forall>i<length (cl1@[the (fst (find_0indegree_blocks al1 bl1 cl1))]). 
      get_block_indegree ((cl1@[the (fst (find_0indegree_blocks al1 bl1 cl1))]) ! i) al1 
      - Outputs (take i (cl1@[the (fst (find_0indegree_blocks al1 bl1 cl1))])) = {}"
      using 2(2,7) by (smt (verit) False Suc_leI append1_eq_conv append_Nil2 diff_is_0_eq 
          find_0indegree_block_lemma length_append_singleton less_Suc_eq nth_append 
          nth_append_length option.collapse prod.exhaust_sel take_all_iff take_append 
          take_eq_Nil take_hd_drop)
    have 11: "(snd (find_0indegree_blocks al1 bl1 cl1))@
        (cl1 @ [the (fst (find_0indegree_blocks al1 bl1 cl1))]) <~~> c@(cl2@[b])"
    proof -
      have tmp1: "c@cl2@[b] <~~> bl2@cl2"
        using 7 find_0indegree_block_lemma2 find_0indegree_lemma4 False perm_remove3 by (metis "6" 
            find_0indegree_lemma3 list.set_intros(1) option.collapse perm_sym prod.exhaust_sel)
      then show ?thesis using 4 2(3) by (meson perm.trans perm_sym)
    qed
    have 12: "length (topological_sort al1 bl1 cl1) = length (topological_sort al1 
      (snd (find_0indegree_blocks al1 bl1 cl1)) (cl1 @ [the (fst (find_0indegree_blocks al1 bl1 cl1))]))"
      using topological_sort.simps[of al1 bl1 cl1] False by force
    have 13: "length (topological_sort al2 bl2 cl2) = length (topological_sort al2 c (cl2 @ [b]))"
      using 7 topological_sort.simps[of al2 bl2 cl2] "6" by force
    have 14: "length (topological_sort al1 (snd (find_0indegree_blocks al1 bl1 cl1)) 
    (cl1 @ [the (fst (find_0indegree_blocks al1 bl1 cl1))])) = length (topological_sort al2 c (cl2 @ [b]))"
      using 2(1,2) 3 11 8 5 9 10 12 by blast
    then show ?thesis using 12 13 by auto
  qed
qed
...
lemma loop_free_perm: "Unique bl1 \<Longrightarrow> bl1 <~~> bl2 \<Longrightarrow> loop_free bl1 \<Longrightarrow> loop_free bl2"
  subgoal premises pre 
  proof -
    have 1: "bl1 <~~> topological_sort bl1 bl1 []"
      using pre sort_perm unfolding sortDiag_def by auto
    have 2: "length (topological_sort bl1 bl1 []) = length (topological_sort bl2 bl2 [])"
      using pre toposort_perm2 by simp
    show ?thesis using pre 1 2 by (simp add: perm_length sortDiag_def)
  qed
  done

lemma sortDiag_Unique: "Unique bl \<Longrightarrow> Unique (sortDiag bl)"
  unfolding sortDiag_def using toposort_Unique by auto

lemma sortDiag_subset: "set (sortDiag bl) \<subseteq> set bl"
  using sort_subset2 unfolding sortDiag_def by (metis append.right_neutral)

\<comment> \<open>End section\<close>

\<comment> \<open>timed_vars is an important definition, recording all variables values at all time\<close>
...
section \<open>the following functions are defined for computing sample time\<close>
