...
lemma perm_neq: "\<not> (xs <~~> ys) \<Longrightarrow> length xs = length ys \<Longrightarrow> (\<exists>j < length xs. (xs ! j \<noteq> ys ! j \<and> 
(\<forall>i<j. xs ! i = ys ! i)))"
...
next
  ...
  proof(cases "hd ys = a")
    case True
    have 1: "\<exists>j<length xs. xs ! j \<noteq> (tl ys) ! j \<and> (\<forall>i<j. xs ! i = (tl ys) ! i)"
      using Cons True by (metis hd_Cons_tl length_0_conv length_tl list.sel(3) perm.Cons)
    then show ?thesis using True by (smt (verit, best) Cons.prems(2) length_0_conv length_Cons 
          less_Suc_eq_0_disj list.collapse list.sel(3) nth_Cons_0 nth_Cons_Suc)
  next
    case False
    then show ?thesis by (metis Cons.prems(2) hd_conv_nth impossible_Cons length_0_conv 
          length_greater_0_conv less_eq_nat.simps(1) not_less_zero nth_Cons_0)
  qed
qed
