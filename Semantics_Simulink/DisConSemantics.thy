...
\<comment> \<open>The file firstly defines the constraints and executions for continuous diagrams, then
proof the existence and uniqueness for continuous diagrams. we also define 
the constraints and executions for mixed diagrams(containing discrete and continuous blocks).
, then proof the existence and uniqueness for mixed diagrams

Finally, we define the syntax and execution for subsystem(need to redefined), and if 
there is any difficult in the future, please do not hesitate to contact me!\<close>

section \<open>lemmas for ODE solutions\<close>
...
lemma has_vderiv_on_subset:
  assumes "(p has_vderiv_on q) D1"
  and "D2 \<subseteq> D1"
  shows "(p has_vderiv_on q) D2"
  using assms Vector_Derivative_On.has_vderiv_on_subset by auto
...
lemma vector_real_derivative: 
  assumes "\<forall>t. ((\<lambda>t. p t $ i) has_vector_derivative 0) (at t within {a<--<b})"
  shows "\<forall>t. ((\<lambda>t. p t $ i) has_real_derivative 0) (at t within {a<--<b})"
  using assms has_field_derivative_iff_has_vector_derivative[of "(\<lambda>t. p t $ i)" 0]
  unfolding has_vector_derivative_def by presburger

lemma has_real_derivative_zero:
  assumes "\<forall>t \<in> {a--b}. (p has_real_derivative 0) (at t within {a--b})"
      and "a < b" and "continuous_on {a..b} p"
    shows "tt \<in> {a--b} \<Longrightarrow> p tt = p a "
  subgoal premises pre 
proof -
  have 1: "tt \<ge> a \<and> tt \<le> b"
    using pre unfolding closed_segment_def by (smt (verit, ccfv_SIG) DiffD1 
        DiffD2 assms(2) atLeastAtMost_iff box_real(2) convex_hull_eq_real_cbox 
        convex_hull_insert_alt equals0D insertCI mem_Collect_eq)
  have 2: "\<And>x. a < x \<Longrightarrow> x < b \<Longrightarrow> (p has_real_derivative 0) (at x)"
    using assms(1) subgoal premises pre2 for x 
    proof -
      have tmp1: "{a--b} = {a..b}"
        using assms(2) real_Icc_closed_segment by auto
      show ?thesis using  at_within_Icc_at[of a x b] pre2 tmp1 assms(1)
        by (smt (verit, best) atLeastAtMost_iff)
    qed
    done
  show ?thesis using assms 1 2 DERIV_isconst2[of a b p tt] by simp
qed
  done

lemma has_vector_derivative_within: 
  assumes "(p1 has_vector_derivative q t) (at t within D)" 
     and "\<forall>t \<in> D. p1 t = p2 t " "t \<in> D"
   shows "(p2 has_vector_derivative q t) (at t within D)"
  using assms unfolding has_vector_derivative_def  using has_derivative_transform[of t D p1 p2 "(\<lambda>x. x *\<^sub>R q t)"]
  by (metis has_derivative_transform)

lemma has_vector_derivative_within_ith:
  assumes "((\<lambda>t. p1 t $ i) has_vector_derivative q1) (at t within D)" 
     and "\<forall>t \<in> D. p1 t $ i = p2 t $ i" "q1 = q2" "t \<in> D" 
   shows "((\<lambda>t. p2 t $ i) has_vector_derivative q2) (at t within D)"
  by (metis (no_types, lifting) assms(1) assms(2) assms(3) assms(4) has_vector_derivative_transform)
...
lemma solveODE_att0: "\<exists>L. unique_on_bounded_closed t0
 {t0 -- t0+ t} (state2vec (timedVar2State h t0 )) (block2ODE b) UNIV L \<Longrightarrow>
\<forall>v. (solveODE h b t0 t) t0 $ v = h v t0"
  using unique_on_bounded_closed.fixed_point_iv unfolding solveODE_def 
  by (metis state2vec_def timedVar2State_def vec_lambda_beta)
...
\<comment> \<open>End sction\<close>


section \<open>definitions and theorems for constraints\<close>
...
\<comment> \<open>constraints for combined ODE\<close>
...
fun checkOneBlockODE :: "var list \<Rightarrow> var list \<Rightarrow> outupd list \<Rightarrow> 
timed_vars \<Rightarrow> real \<Rightarrow> real \<Rightarrow> bool" where
  "checkOneBlockODE il [] fl h t0 t = True" |
  "checkOneBlockODE il vl [] h t0 t = True" |
  "checkOneBlockODE il (v#vl) (f#fl) h t0 t = (checkOneBlockODE il vl fl h t0 t \<and>
  ((\<lambda>t. (realstate2realvec (timedVar2timedState h)) t $ v)
has_vderiv_on (\<lambda>t. f (map (\<lambda> a. h a t) il))) {t0<--<t0 + t})"

lemma checkOneBlockODE_eq: "length vl = length fl \<Longrightarrow> \<forall>i. i \<ge> 0 \<and> i < length vl \<longrightarrow>
((\<lambda>t. (realstate2realvec (timedVar2timedState h)) t $ (vl ! i))
has_vderiv_on (\<lambda>t. (fl ! i) (map (\<lambda> a. h a t) il))) {t0<--<t0 + t} \<Longrightarrow>
checkOneBlockODE il vl fl h t0 t"
...
next
  ...
  have 2: "checkOneBlockODE il vl (tl fl) h t0 t"
  proof -
    have tmp1: "length vl = length (tl fl)"
      using 1 Cons(2) by simp
    have tmp2: "\<forall>i. 0 \<le> i \<and> i < length vl \<longrightarrow>
        ((\<lambda>t. realstate2realvec (timedVar2timedState h) t $ vl ! i) has_vderiv_on
         (\<lambda>t. (tl fl ! i) (map (\<lambda>a. h a t) il)))
         {t0<--<t0 + t}"
      apply clarify subgoal premises pre2 for i
      proof -
        show ?thesis using pre2 Cons(3) by (smt (verit, best) Suc_leI has_vderiv_on_cong 
              le_imp_less_Suc length_Cons less_imp_le_nat list.sel(3) nth_tl tmp1)
      qed
      done
    show ?thesis using Cons(1)[of "tl fl"] tmp2 tmp1 by simp
  qed
  have 3: "{t0<--<t0 + t} \<subseteq> {t0--t0 + t}"
    unfolding open_segment_def by simp
  show ?case using checkOneBlockODE.simps(3)[of il v vl "hd fl" "tl fl" h t0 t] 1 2 Cons(3) 
    by (metis bot_nat_0.extremum length_greater_0_conv list.distinct(1) nth_Cons_0)
qed

lemma has_vderiv_on_eq1: "(\<lambda>t. p t $ i) = (\<lambda>t. 
(apply_bcontfun (unique_on_bounded_closed.fixed_point t0 T x0 f X)) t $ i) \<Longrightarrow>
unique_on_bounded_closed t0 T x0 f X L \<Longrightarrow> ((\<lambda>t. p t $ i) has_vderiv_on 
(\<lambda>t. f t (apply_bcontfun (unique_on_bounded_closed.fixed_point t0 T x0 f X) t) $ i)) T"
  using unique_on_bounded_closed.fixed_point_solution[of t0 T x0 f X L] 
  unfolding solves_ode_def using has_vderiv_on_proj
  by (smt (verit, best) has_vderiv_eq)
...
lemma behavCalBlk_ith: "behavCalBlk il vl ol fl t0 t h \<Longrightarrow> length vl = length fl \<and> 
length vl = length ol \<Longrightarrow> x > t0 \<and> x < t0 + t \<Longrightarrow> set ol = {0}
\<Longrightarrow> \<forall>i. i \<ge> 0 \<and> i < length vl \<longrightarrow> h (vl ! i) x = (fl ! i) (map (\<lambda>a. h a x) il)"
  apply clarify subgoal premises pre for i
    using pre
  proof(induction vl arbitrary: fl ol i)
    case Nil
    then show ?case by auto
  next
    case (Cons v vl)
    have 1: "fl = hd fl # (tl fl)"
      using Cons(4) by (metis length_0_conv list.distinct(1) list.exhaust_sel)
    have 2: "ol = hd ol # (tl ol)"
      using Cons(5) by (metis length_0_conv list.distinct(1) list.exhaust_sel)
    then show ?case
    proof(cases "i = 0")
      case True
      then show ?thesis using behavCalBlk.simps(4)[of il v vl "hd ol" "tl ol" "hd fl" "tl fl"
            t0 t h] using Cons(2,3,6,7) by (metis "1" "2" One_nat_def add.right_neutral 
            empty_iff insert_iff length_0_conv list.distinct(1) list.size(4) nth_Cons_0)
    next
      case False
      have 3: "i \<ge> 1"
        using Cons(8) False by simp
      have 4: "behavCalBlk il vl (tl ol) (tl fl) t0 t h"
        using Cons(2) 1 2 behavCalBlk.simps(4)[of il v vl "hd ol" "tl ol" "hd fl" "tl fl" t0 t h]
        using Cons.prems(2) pre(5) pre(6) by auto
      have 5: "set (tl ol) = {0}"
        using Cons(3) 2 by (metis "3" Cons.prems(4) Cons.prems(8) One_nat_def Suc_leI length_Cons 
            list.size(3) not_less_eq_eq set_empty2 set_subset_Cons subset_singletonD)
      then show ?thesis using Cons(1)[of "tl ol" "tl fl" "i-1"] Cons(4,5,6,7,8,9) 3 4 5 
        by (metis (no_types, lifting) "1" False bot_nat_0.extremum le_add_diff_inverse length_Cons 
            length_tl list.sel(3) nat_add_left_cancel_less nth_Cons' plus_1_eq_Suc)
  qed
qed
done
...
lemma behavCalBlks_valeq: "behavCalBlks bl t0 t h \<Longrightarrow> x < t0 + t \<and> x > t0 \<Longrightarrow> wf2 bl \<Longrightarrow>
\<forall>b\<in>set bl. set (get_offsets b) = {0} \<Longrightarrow>
\<forall>b\<in>set bl. \<forall>i. 0 \<le> i \<and> i < length (get_outputs b) \<longrightarrow>
              h (get_outputs b ! i) x = (get_outupd b ! i) (map (\<lambda>a. h a x) (get_inputs b))"
  apply clarify subgoal premises pre for b i
    using pre
  proof(induction bl)
    case Nil
    then show ?case by simp
  next
    ...
    proof(cases "b=a")
      case True
      have 1: "behavCalBlk (get_inputs b) (get_outputs b) (get_offsets b) (get_outupd b) t0 t h"
        using Cons(2) unfolding behavCalBlks.simps True by simp
      have 2: "length (get_outputs b) = length (get_outupd b) \<and>
        length (get_outputs b) = length (get_offsets b)"
        using Cons(3) True unfolding wf2_def Available'_def using Cons.prems(4) by presburger
      have 3: "set (get_offsets b) = {0}"
        using Cons(4) True by simp
      then show ?thesis using behavCalBlk_ith[of "(get_inputs b)" "(get_outputs b)" 
            "(get_offsets b)" "(get_outupd b)"  t0 t h x] 1 2 Cons by auto
    next
      case False
      then show ?thesis using Cons wf2_lemma by simp
    qed
  qed
  done
...
\<comment> \<open>note: root constraints!!!\<close>
fun behavConSingle:: "block list \<Rightarrow> real \<Rightarrow> real \<Rightarrow> timed_vars \<Rightarrow> timed_vars \<Rightarrow> bool" where
  "behavConSingle bl t0 t h h0 = (\<forall>b \<in> set (findIntegBlks bl). checkOneBlockODE 
  (get_inputs b) (get_outputs b) (get_outupd b) h t0 t \<and> behavCalBlks (getCalBlks bl) t0 t h)"
...
section \<open>definitions and theorems for executing a continuous diagrams\<close>
...
\<comment> \<open>note: root execution!!!\<close>
        ...
        proof -
          ...
          then show ?thesis using exeCalBlk_lemma1 using tmp1 1 2 False
             using exeCalBlk.simps(4)[of il v vl "hd ol" "tl ol" "hd fl"
              "tl fl" h t0 t] by (smt (verit, del_insts))
        qed
...
lemma checkODE_forSingle: "wf2 bl \<Longrightarrow>
b = updateIntegBlks bl (findIntegBlks bl) \<Longrightarrow>
cl = findIntegBlks bl \<Longrightarrow> checkODE b h h0 t0 t \<Longrightarrow>
behavCalBlks (getCalBlks bl) t0 t h \<Longrightarrow>
\<forall>x \<in> Outputs bl. h x t0 = h0 x t0 \<Longrightarrow> t > 0 \<Longrightarrow>
ODE_cond (updateIntegBlks bl (findIntegBlks bl)) h0 t0 t = True \<Longrightarrow>
\<forall>x \<in> Inputs bl - Outputs bl. \<forall>tt. tt > t0 \<and> tt < t0 + t \<longrightarrow> h x tt = h0 x t0 \<Longrightarrow>
loop_free (getCalBlks bl) \<Longrightarrow>
\<forall>c \<in> set cl. checkOneBlockODE (get_inputs c) (get_outputs c) (get_outupd c) h t0 t = True"
  apply clarify subgoal premises pre for c
  proof -
    have 1: "Outputs cl \<subseteq> set (get_outputs b)"
    proof -
      have tmp1: "wf2 cl"
        using pre(1,2) findIntegBlkswf by presburger
      have tmp2: "\<forall>b\<in>set bl. set (get_offsets b) = {1} \<longrightarrow> 
       set (get_outputs b) \<subseteq> Outputs (findIntegBlks bl)"
        by (metis Outputs.simps(2) Un_iff findIntegBlks.simps(2) 
            findIntegBlksSubset2 insert_absorb list.simps(15) subsetI subset_trans)
      have tmp3: "\<forall>c\<in>set (findIntegBlks bl). set (get_offsets c) = {1}"
      ...
      have tmp4: "set (findIntegBlks bl) \<subseteq> set bl"
        using findIntegBlksSubset3 by presburger
      show ?thesis using pre(1,2,4,5,6,7,8,9,10) tmp1 tmp2 tmp3 tmp4 
          updateIntegBlksSubset2[of bl "findIntegBlks bl"] by simp
    qed
    have ODEValue_eq: "(\<forall>i \<in> Outputs cl. \<forall>h2. (\<lambda>t. (realstate2realvec (timedVar2timedState h)) 
    t $ i) = (\<lambda>t'. (realstate2realvec (timedVar2timedState 
   (\<lambda>vv tt. (if vv \<in> set (get_outputs b) then h vv tt else 
    (h2 vv tt))))) t' $ i) )"
    using 1 trans_eq6 by (simp add: in_mono timedVar2timedState_def)
    have 2: "\<forall>i. ((\<lambda>ta. realstate2realvec
            (timedVar2timedState
              (\<lambda>vv tt.
                  if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and>
                     t0 < tt \<and> tt \<le> t0 + t
                  then h vv tt
                  else timedState2timedVar
                        (realvec2realstate
                          (apply_bcontfun (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
                        vv tt))
            ta $
           i) has_vderiv_on
     (\<lambda>ta. block2ODE (updateIntegBlks bl (findIntegBlks bl)) ta
            (realstate2realvec
              (timedVar2timedState
                (\<lambda>vv tt.
                    if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and>
                    ...
                    then h vv tt
                          ...
                          (realvec2realstate
                            (apply_bcontfun
                              (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
                          vv tt))
              ta) $
           i))
     {t0--t0 + t}"
      apply clarify subgoal for i
      using has_vderiv_on_proj[of "(realstate2realvec
      (timedVar2timedState
        (\<lambda>vv tt.
            if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and> t0 < tt \<and> tt \<le> t0 + t
            then h vv tt
            else timedState2timedVar
                  (realvec2realstate
                    (apply_bcontfun (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
                  vv tt)))" "(\<lambda>ta. block2ODE (updateIntegBlks bl (findIntegBlks bl)) ta
            (realstate2realvec
              (timedVar2timedState
                (\<lambda>vv tt.
                    if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and>
                    ...
                    then h vv tt
                          ...
                          (realvec2realstate
                            (apply_bcontfun
                              (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
                          vv tt))
              ta))" "{t0--t0 + t}" i] pre(3) unfolding checkODE.simps solves_ode_def by auto
    done
  have 3: "length (get_outputs c) = length (get_outupd c)"
    using pre unfolding wf2_def using Available'_def findIntegBlkswf wf2_def by blast
  have 4: "\<forall>i. 0 \<le> i \<and> i < length (get_outputs c) \<longrightarrow>
    ((\<lambda>t. realstate2realvec (timedVar2timedState h) t $ (get_outputs c) ! i) has_vderiv_on
     (\<lambda>t. ((get_outupd c) ! i) (map (\<lambda>a. h a t) (get_inputs c))))
     {t0<--<t0+t}"
    unfolding has_vderiv_on_def apply clarify subgoal premises pre4 for i x
    proof -
      have tmp1: "\<forall>ta\<in>{t0--t0 + t}.
       realstate2realvec
        (timedVar2timedState
          (\<lambda>vv tt.
              if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and>
        ...
        ta $
       get_outputs c ! i =
       realstate2realvec (timedVar2timedState h) ta $ get_outputs c ! i"
      proof -
        have 0: "set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) = Outputs (findIntegBlks bl)"
        proof -
          have tmp1: "wf2 (findIntegBlks bl)"
          ...
          qed
          have tmp3: "\<forall>b\<in>set bl. set (get_offsets b) = {1} \<longrightarrow> 
          set (get_outputs b) \<subseteq> Outputs (findIntegBlks bl)"
          ...
          show ?thesis using updateIntegBlksSubset2 pre(1) tmp1 tmp2 tmp3 
            findIntegBlksSubset3 updateIntegBlksSubset by presburger
        qed
        have 00: "get_outputs (updateIntegBlks bl (findIntegBlks bl)) \<noteq> []"
        proof -
          have tmp1: "get_outputs c \<noteq> []"
            using pre4 by force
          have tmp2: "Outputs (findIntegBlks bl) \<noteq> {}"
            using pre(9,10,11) tmp1 by (metis Outputs_base3 Un_iff ex_in_conv nth_mem pre4(3))
          show ?thesis using tmp2 0 by auto
        qed
        have 1: "get_outputs c ! i \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl)))"
          using 0 pre(9,10,11) by (metis Outputs_base2 Un_iff nth_mem pre4(3))
        have 2: "\<forall>v. (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t) t0 $ v = h0 v t0"
          using solveODE_att0 pre(7) unfolding ODE_cond_def by presburger
        have 3: "\<forall>ta \<in>{t0--t0+t}. ta \<ge> t0 \<and> ta \<le> t0+t"
          unfolding closed_segment_def real_scaleR_def apply simp apply clarify subgoal for ta u 
            using pre(6) by (simp add: left_diff_distrib ring_class.ring_distribs(1))
          done
        have 4: "set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<subseteq> Outputs bl"
          using 0 findIntegBlksSubset by blast
        show ?thesis apply clarify subgoal premises pre1 for ta
          proof(cases "ta = t0")
            case True
            then show ?thesis using 1 2 4 pre(5) 
              by (smt (z3) state2vec_def subsetD timedVar_state_map4 trans_eq5 vec_lambda_beta)
          next
            case False
            have 5: "ta > t0 \<and> ta \<le> t0+t"
              using False pre1 3 by fastforce
            then show ?thesis using 1 by (smt (verit) trans_eq3)
          qed
          done
      qed
      have tmp2: "block2ODE (updateIntegBlks bl (findIntegBlks bl)) x
   (realstate2realvec
     (timedVar2timedState
       (\<lambda>vv tt.
           if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and> t0 < tt \<and> tt \<le> t0 + t
     ...
     x) $
  get_outputs c ! i =
  (get_outupd c ! i) (map (\<lambda>a. h a x) (get_inputs c))"
      proof -
        have tmp2_1: "\<exists>k. get_outputs c ! i = 
          (get_outputs (updateIntegBlks bl (findIntegBlks bl)) ! k) \<and>
      block2ODE (updateIntegBlks bl (findIntegBlks bl)) x
   (realstate2realvec
     (timedVar2timedState
       (\<lambda>vv tt.
           if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and> t0 < tt \<and> tt \<le> t0 + t
     ...
     x) $
  get_outputs c ! i = 
  (get_outupd (updateIntegBlks bl (findIntegBlks bl)) ! k) 
  (map (\<lambda>a. h a x) (get_inputs (updateIntegBlks bl (findIntegBlks bl)))) \<and>
  (get_outupd (updateIntegBlks bl (findIntegBlks bl)) ! k) 
        (map (\<lambda>a. h a x) (get_inputs (updateIntegBlks bl (findIntegBlks bl)))) 
        = (get_outupd c ! i) (map (\<lambda>a. h a x) (get_inputs c))"
        proof - 
          have tmp2_0: "set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) = Outputs (findIntegBlks bl)"
        proof -
          have tmp1: "wf2 (findIntegBlks bl)"
            using pre(1) findIntegBlkswf by auto
          have tmp2: "\<forall>c\<in>set (findIntegBlks bl). set (get_offsets c) = {1}"
          ...
          show ?thesis using updateIntegBlksSubset2 pre(1) tmp1 tmp2 tmp3 
            findIntegBlksSubset3 updateIntegBlksSubset by presburger
          qed
          obtain k::nat where tmp2_1: "(get_outputs c ! i) = 
          (get_outputs (updateIntegBlks bl (findIntegBlks bl)) ! k) \<and> 
          k \<ge> 0 \<and> k < length (get_outputs (updateIntegBlks bl (findIntegBlks bl)))"
            using tmp2_0 pre(9,10,11) in_set_conv_nth by (metis Outputs_base2 Un_iff nth_mem pre4(3) zero_le)
          have tmp2_2: "(get_outupd (updateIntegBlks bl (findIntegBlks bl)) ! k) 
        (map (\<lambda>a. h a x) (get_inputs (updateIntegBlks bl (findIntegBlks bl)))) 
        = (get_outupd c ! i) (map (\<lambda>a. h a x) (get_inputs c))"
          proof -
            have tmp1: "wf2 (findIntegBlks bl)"
              using pre(1) findIntegBlkswf by force
            have tmp2: "\<forall>b\<in>set (getCalBlks bl). \<forall>i. 0 \<le> i \<and> i < length (get_outputs b) \<longrightarrow>
              h (get_outputs b ! i) x = (get_outupd b ! i) (map (\<lambda>a. h a x) (get_inputs b))"
              apply clarify subgoal premises pre2 for b i
              proof -
                have tmp2_1: "x < t0 + t \<and> t0 < x"
                  using pre4(1) unfolding open_segment_def closed_segment_def real_scaleR_def 
                  apply simp apply clarify subgoal for ta  
                    using pre(6) by (simp add: left_diff_distrib ring_class.ring_distribs(1))
                  done
                have tmp2_2: "wf2 (getCalBlks bl)"
                  using getCalBlkswf pre(1) by simp
                have tmp2_3: "\<forall>b\<in>set (getCalBlks bl). set (get_offsets b) = {0}"
                ...
                show ?thesis using pre(4) behavCalBlks_valeq[of "(getCalBlks bl)" t0 t h x] 
                  unfolding behavCalBlks.simps using tmp2_1 tmp2_2 tmp2_3 pre2(1) pre2(3) by blast
              qed
              done
            show ?thesis using updateIntegBlks_funeq[of bl "findIntegBlks bl" h x c i k] 
              pre(1,9,10,11) tmp1 tmp2 tmp2_1 findIntegBlksSubset3 pre4(3) findIntegBlks_forall 
              by presburger
          qed
          have tmp2_3: "block2ODE (updateIntegBlks bl (findIntegBlks bl)) x
   (realstate2realvec
     (timedVar2timedState
       (\<lambda>vv tt.
           if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and> t0 < tt \<and> tt \<le> t0 + t
          ...
          proof -
            have 0: "set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) = Outputs (findIntegBlks bl)"
              ...
              proof -
                ...
                show ?thesis using tmp2 0 by auto
              qed
            have tmp1: "length (get_outputs (updateIntegBlks bl (findIntegBlks bl))) =
              length (get_outupd (updateIntegBlks bl (findIntegBlks bl)))"
              using updateIntegBlks_wf2 pre(1) findIntegBlkswf by auto
            have tmp2: "\<forall>i j. i < j \<and> j < length (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and> 0 \<le> j \<longrightarrow>
        get_outputs (updateIntegBlks bl (findIntegBlks bl)) ! i \<noteq>
        get_outputs (updateIntegBlks bl (findIntegBlks bl)) ! j"
              using updateIntegBlks_wf2 pre(1) findIntegBlkswf by auto
            have tmp3: "(if get_outputs (updateIntegBlks bl (findIntegBlks bl)) ! k
      \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and>
      t0 < x \<and> x \<le> t0 + t
   then h (get_outputs (updateIntegBlks bl (findIntegBlks bl)) ! k) x
   else timedState2timedVar
         (realvec2realstate
           (apply_bcontfun (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
         (get_outputs (updateIntegBlks bl (findIntegBlks bl)) ! k) x) =
  h (get_outputs (updateIntegBlks bl (findIntegBlks bl)) ! k) x"
            proof -
              have tmp: "x < t0 + t \<and> t0 < x"
                  using pre4(1) unfolding open_segment_def closed_segment_def real_scaleR_def 
                  apply simp apply clarify subgoal for ta  
                    using pre(6) by (simp add: left_diff_distrib ring_class.ring_distribs(1))
                  done
                then show ?thesis using tmp2_1 pre4(3) 00
                  by (smt (verit, best) linorder_not_le list.size(3) nth_mem pre4(2))
            qed
            have tmp4: "0 \<le> k \<and> k < length (get_outputs (updateIntegBlks bl (findIntegBlks bl)))"
            proof -
              have 1: "get_outputs c \<noteq> []"
                using pre4(3) by force
              show ?thesis using tmp2_1 pre4(3) 00 1 by force
            qed
            have tmp5: "\<forall>xa\<in>set (get_inputs (updateIntegBlks bl (findIntegBlks bl))).
     (if xa \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and> t0 < x \<and> x \<le> t0 + t
      then h xa x
      else timedState2timedVar
            (realvec2realstate
              (apply_bcontfun (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
            xa x) =
     h xa x"
              apply clarify subgoal premises pre5 for xa
              proof(cases "xa \<in> set (get_inputs (updateIntegBlks bl (findIntegBlks bl))) \<inter>
                set (get_outputs (updateIntegBlks bl (findIntegBlks bl)))")
                case True
                have 1: "x < t0 + t \<and> t0 < x"
                  using pre4(1) unfolding open_segment_def closed_segment_def real_scaleR_def 
                  apply simp apply clarify subgoal for ta  
                    using pre(6) by (simp add: left_diff_distrib ring_class.ring_distribs(1))
                  done
                then show ?thesis using True by (simp add: nth_Cons')
              next
                case False
                have 1: "x < t0 + t \<and> t0 < x"
                  using pre4(1) unfolding open_segment_def closed_segment_def real_scaleR_def 
                ...
                have 2: "(if xa \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and> t0 < x \<and> x \<le> t0 + t
      then h xa x
      else timedState2timedVar
            (realvec2realstate
              (apply_bcontfun (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
            xa x) = h0 xa t0"
                proof -
                  have tmp1: "(if xa \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and> t0 < x \<and> x \<le> t0 + t
      then h xa x
      else timedState2timedVar
            (realvec2realstate
              (apply_bcontfun (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
            xa x) = (apply_bcontfun (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)) x $ xa"
                    using False unfolding timedState2timedVar_def realvec2realstate.simps 
                    using pre5 vec2state_def by auto
                  have tmp2: "\<forall>t v1. block2ODE (updateIntegBlks bl (findIntegBlks bl)) t v1 $ xa = 0"
                    using blockODE_zero updateIntegBlks_wf2 findIntegBlkswf by (metis False 
                   Int_iff get_outputs.simps get_outupd.simps get_sample_time.cases pre(1) pre5)
                  have tmp3: "((unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
                              (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV) 
                  solves_ode (block2ODE (updateIntegBlks bl (findIntegBlks bl)))) {t0 -- t0 + t} UNIV"
                    using unique_on_bounded_closed.fixed_point_solution pre(7) unfolding ODE_cond_def by blast
                  have tmp4: "\<forall>tt1 tt2. block2ODE (updateIntegBlks bl (findIntegBlks bl)) tt1
                  (apply_bcontfun
                    (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
                      (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV)
                    tt2) $ xa  = 0"
                    using tmp2 by simp
                  have tmp5: "\<forall>tt\<in> {t0--t0 + t}. ((\<lambda>ta. apply_bcontfun
            (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
              (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV) ta $ xa) 
                has_vector_derivative 0) (at tt within {t0--t0 + t})"
                    apply clarify subgoal premises pre5 for tt
                    proof -
                      have 1: "(apply_bcontfun
        (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
          (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV) has_vector_derivative
       block2ODE (updateIntegBlks bl (findIntegBlks bl)) tt
        (apply_bcontfun
          (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
            (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV)
          tt))
       (at tt within {t0--t0 + t})"
                        using tmp3 pre5 unfolding solves_ode_def has_vderiv_on_def by blast
                    show ?thesis using 1 has_vector_derivative_proj[of "apply_bcontfun
            (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
              (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV)" "\<lambda>tt. 
                block2ODE (updateIntegBlks bl (findIntegBlks bl)) tt (apply_bcontfun
                    (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
                      (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV)
                    tt)" tt  "{t0--t0 + t}" xa] tmp4 unfolding solves_ode_def has_vderiv_on_def
                      by fastforce
                  qed
                  done
                  have tmp6: "\<forall>tt\<in> {t0--t0 + t}. ((\<lambda>ta. apply_bcontfun
            (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
              (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV) ta $ xa) 
                has_real_derivative 0) (at tt within {t0--t0 + t})"
                    using has_field_derivative_iff_has_vector_derivative tmp5 by presburger
                  have tmp7: "continuous_on {t0--t0 + t} (\<lambda>ta. apply_bcontfun
            (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
              (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV) ta $ xa)"
                    using unique_on_bounded_closed.fixed_point_domain by (auto intro!: continuous_intros)
                  have tmp8: "apply_bcontfun
                   (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
                     (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV)
                   x $ xa =
                  apply_bcontfun
                   (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
                     (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV)
                   t0 $ xa"
                    using has_real_derivative_zero[of t0 "t0 + t" "(\<lambda>ta. apply_bcontfun
            (unique_on_bounded_closed.fixed_point t0 {t0--t0 + t} (state2vec (timedVar2State h0 t0))
              (block2ODE (updateIntegBlks bl (findIntegBlks bl))) UNIV) ta $ xa)" x] tmp6 tmp7 
                    by (smt (verit, best) "1" ODE_Auxiliarities.closed_segment_eq_real_ivl open_closed_segment pre4(1))
                  show ?thesis using  tmp1 1 tmp2 tmp8  using ODE_cond_def pre(7) 
                      solveODE_att0 solveODE_def by force
                qed
                have 3: "h xa x = h0 xa t0"
                  using 1 updateIntegBlks_inputsSubset[of bl] pre(1,8,9) pre5 False by blast
                then show ?thesis using 2 by simp
              qed
              done
            have tmp6: "get_outputs c ! i = get_outputs (updateIntegBlks bl (findIntegBlks bl)) ! k"
              using tmp2_1 by blast
            have tmp7: "(Block (get_inputs (updateIntegBlks bl (findIntegBlks bl)))
       (get_outputs (updateIntegBlks bl (findIntegBlks bl)))
       (get_offsets (updateIntegBlks bl (findIntegBlks bl)))
       (get_sample_time (updateIntegBlks bl (findIntegBlks bl)))
       (get_outupd (updateIntegBlks bl (findIntegBlks bl)))) = 
            (updateIntegBlks bl (findIntegBlks bl))"
              by (smt (verit, del_insts) get_inputs.simps get_offsets.simps get_outputs.simps 
                  get_outupd.simps get_sample_time.cases get_sample_time.simps)
              show ?thesis using block2ODE_eqFun[of "get_outputs (updateIntegBlks bl (findIntegBlks bl))" 
                  "get_outupd (updateIntegBlks bl (findIntegBlks bl))" "(\<lambda>vv tt.
             if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and>
                t0 < tt \<and> tt \<le> t0 + t
             then h vv tt
             else timedState2timedVar
                   (realvec2realstate
                     (apply_bcontfun (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
                   vv tt)" k x h "(get_inputs (updateIntegBlks bl (findIntegBlks bl)))"
                "get_offsets (updateIntegBlks bl (findIntegBlks bl))"
                "get_sample_time (updateIntegBlks bl (findIntegBlks bl))"] 
                using tmp1 tmp2 tmp3 tmp4 tmp5 tmp6 tmp7 by fastforce
            qed
          then show ?thesis using tmp2_1 tmp2_2 by auto
        qed
        show ?thesis using tmp2_1 by force
      qed
      have tmp3: "((\<lambda>ta. realstate2realvec
            (timedVar2timedState
              (\<lambda>vv tt.
                  if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and>
                     t0 < tt \<and> tt \<le> t0 + t
                  then h vv tt
                        ...
                        vv tt))
            ta $
           get_outputs c ! i) has_vector_derivative
     block2ODE (updateIntegBlks bl (findIntegBlks bl)) x
      (realstate2realvec
        (timedVar2timedState
          (\<lambda>vv tt.
     ...
     get_outputs c ! i)
     (at x within {t0--t0 + t})"
        using 2 unfolding has_vderiv_on_def using pre4 open_segment_def by blast
      show ?thesis using has_vector_derivative_within_ith[of "realstate2realvec (timedVar2timedState (\<lambda>vv tt.
                  if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and>
                     t0 < tt \<and> tt \<le> t0 + t
                  then h vv tt
                  else timedState2timedVar
                        (realvec2realstate
                          (apply_bcontfun (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
                        vv tt))" "(get_outputs c ! i)" "block2ODE (updateIntegBlks bl (findIntegBlks bl)) x
              (realstate2realvec
                (timedVar2timedState
                  (\<lambda>vv tt.
                      if vv \<in> set (get_outputs (updateIntegBlks bl (findIntegBlks bl))) \<and>
                      ...
                      then h vv tt
                            ...
                            (realvec2realstate
                              (apply_bcontfun
                                (solveODE h0 (updateIntegBlks bl (findIntegBlks bl)) t0 t)))
                            vv tt))
                x) $ (get_outputs c ! i)" x "{t0<--<t0+t}" "realstate2realvec (timedVar2timedState h)"
            "((get_outupd c) ! i) (map (\<lambda>a. h a x) (get_inputs c))"] using tmp1 tmp2 tmp3 pre4(1) 
        has_vderiv_on_subset unfolding has_vderiv_on_def open_segment_def
        by (meson Diff_subset has_vector_derivative_within_subset subsetD)
    qed
    done
      show ?thesis using checkOneBlockODE_eq[of "(get_outputs c)" "(get_outupd c)" ] 3 4 by blast
qed
  done

\<comment> \<open>End section\<close>

section \<open>the following lemmas are the key theorems:existence and uniqueness for continuous diagram\<close>
...
\<comment> \<open>important: correctness for combination\<close>
lemma existence_of_continuous_diag_single: 
  shows "wf2 bl \<Longrightarrow> behavConDiag bl t0 t (exeContinuousDiag_interval bl h t0 t) h \<Longrightarrow>
       ODE_cond (updateIntegBlks bl (findIntegBlks bl)) h t0 t = True \<Longrightarrow> t > 0 \<Longrightarrow>
       \<forall>x \<in> Inputs bl - Outputs bl. \<forall>tt. tt > t0 \<and> tt < t0 + t \<longrightarrow> h x tt = h x t0 \<Longrightarrow>
       besorted2 (rev (getCalBlks bl)) \<Longrightarrow> loop_free (getCalBlks bl) \<Longrightarrow>
       behavConSingle bl t0 t (exeContinuousDiag_interval bl h t0 t) h"
  subgoal premises pre
  proof -
    have 1: "\<forall>x \<in> Outputs bl. (exeContinuousDiag_interval bl h t0 t) x t0 = h x t0"
      using exeContinuousDiag_interval_lemma3 pre by simp
    have 2: "\<forall>x \<in> Inputs bl - Outputs bl. \<forall>tt. tt > t0 \<and> tt < t0 + t \<longrightarrow> 
          (exeContinuousDiag_interval bl h t0 t) x tt = h x t0"
      using exeContinuousDiag_interval_lemma1 pre by (metis DiffD2)
    show ?thesis using pre unfolding behavConDiag.simps behavConSingle.simps using 1 2 checkODE_forSingle[where bl = bl and 
        h = "(exeContinuousDiag_interval bl h t0 t)" and ?h0.0 = h and ?t0.0 = t0 and t = t]
      by blast
  qed
  done
  

\<comment> \<open>existence-1 for continuous diagram!!!\<close>
...
\<comment> \<open>existence-2 for continuous diagram!!!\<close>
lemma existence_of_continuous_diag': 
shows "cl = (getCalBlks bl) \<Longrightarrow> sortDiag (rev cl) = rev cl  \<Longrightarrow> wf2 bl \<Longrightarrow> 
       ODE_cond (updateIntegBlks bl (findIntegBlks bl)) h t0 t = True \<Longrightarrow>  t > 0 \<Longrightarrow>
       \<forall>x \<in> Inputs bl - Outputs bl. \<forall>tt. tt \<ge> t0 \<and> tt < t0 + t \<longrightarrow> h x tt = h x t0 \<Longrightarrow>
       besorted2 (rev (getCalBlks bl)) \<Longrightarrow> loop_free(getCalBlks bl) \<Longrightarrow>
       behavConSingle bl t0 t (exeContinuousDiag_interval bl h t0 t) h"
  subgoal premises pre 
    using pre(1,2,3) sort_is_sorted2 existence_of_continuous_diag_single 
      existence_of_continuous_diag_base pre(4,5,6,7,8) by (smt (verit))
  done
...
\<comment> \<open>Uniqueness for continuous diagrams!!!\<close>
lemma uniqueness_of_continuous_diag :
"bl <~~> bl' \<Longrightarrow> cl = (getCalBlks bl) \<Longrightarrow>
cl' = (getCalBlks bl') \<Longrightarrow> besorted2 cl \<Longrightarrow> wf2 bl \<Longrightarrow> besorted2 cl' \<Longrightarrow>
ODE_cond (updateIntegBlks bl (findIntegBlks bl)) h t0 t = True \<Longrightarrow>
...
section \<open>the following functions define the constraints and execution for mixed diagrams\<close>
...
section \<open>the following lemmas are the key theorems:existence and uniqueness\<close>
...
lemma exe_diag_tilltime_eq3: "\<forall>x. x \<notin> Outputs bl \<union> Outputs cl \<longrightarrow> (\<forall>t. h x t = h x 0) \<Longrightarrow>
loop_free (bl @ getCalBlks cl) \<Longrightarrow> wf2 cl \<Longrightarrow>
\<forall>t::nat < k. \<forall>tt::real . \<forall>v\<in> Inputs cl - Outputs cl. tt > t \<and> tt < t + 1 \<longrightarrow>
exe_diag_tilltime bl cl h k v tt = (exe_diag_tilltime bl cl h (k-1)) v t"
  apply clarify subgoal premises pre for t tt v using pre
  proof(induction k)
    case 0
    then show ?case by auto
  next
    ...
    proof(cases "t = k")
      case True
      note T1 = True
      have 1: "exeDisDiag_attime (rev (sortDiag (bl @ getCalBlks cl)))
     (exeContinuousDiag_interval cl (exeDisDiag_interval bl (exe_diag_tilltime bl cl h k) (real k) 1)
       (real k) 1)
     (nat \<lfloor>real k + 1\<rfloor>) v tt = (exeContinuousDiag_interval cl (exeDisDiag_interval bl (exe_diag_tilltime bl cl h k) (real k) 1)
       (real k) 1) v tt"
        using exeDisDiag_attime_lemma2 Suc(8,9) True by auto
      have 2: "(exeContinuousDiag_interval cl (exeDisDiag_interval bl (exe_diag_tilltime bl cl h k) (real k) 1)
       (real k) 1) v tt = (exeDisDiag_interval bl (exe_diag_tilltime bl cl h k) (real k) 1) v tt"
        using Suc(4,6,7) exeContinuousDiag_interval_lemma1 by blast
      have 3: "(exeDisDiag_interval bl (exe_diag_tilltime bl cl h k) (real k) 1) v tt =
        exe_diag_tilltime bl cl h k v (real t)"
      proof(cases "v \<in> Outputs bl")
        case True
        then show ?thesis unfolding exeDisDiag_interval.simps using Suc(8,9) T1 by auto
      next
        case False
        then show ?thesis unfolding exeDisDiag_interval.simps using Suc(2,7) exe_diag_tilltime_eq2 
          using pre(2) pre(3) by (metis UnE)
      qed
      then show ?thesis unfolding exe_diag_tilltime.simps exe_diag_interval.simps 
        using "1" "2" by force
    next
      case False
      have 1: "t < k"
        using False Suc(5) by auto
      have 2: "exe_diag_tilltime bl cl h (Suc k) v tt = 
        exe_diag_tilltime bl cl h k v tt"
      proof -
        have tmp1: "(real k) < real (nat \<lfloor>(real k) + 1\<rfloor>)"
          by simp
        have tmp2: "tt < real k"
          using 1 Suc(8,9) by linarith
        show ?thesis unfolding exe_diag_tilltime.simps using exe_diag_interval_lemma1 tmp1 tmp2 
          by (smt (z3))
      qed
      have 3: "exe_diag_tilltime bl cl h k v (real t) = exe_diag_tilltime bl cl h (k - 1) v (real t)"
      proof -
        have tmp1: "k = Suc (k-1)"
          using 1 by auto
        have tmp2: "(real (k-1)) < real (nat \<lfloor>(real (k-1)) + 1\<rfloor>)"
          by simp
        have tmp3: "t \<le> real (k-1)"
          using 1 Suc(8,9) by linarith
        show ?thesis using exe_diag_tilltime.simps(2)[of bl cl h "k-1"] exe_diag_interval_lemma1 
            tmp1 tmp2 tmp3 by (smt (z3))
      qed
      have 4: "exe_diag_tilltime bl cl h k v tt = exe_diag_tilltime bl cl h (k - 1) v (real t)"
        using 1 Suc by fastforce
      then show ?thesis using 2 3 4 by auto
    qed
  qed
  done
...
definition behavConDiag3 :: "block list \<Rightarrow> block list \<Rightarrow> real \<Rightarrow> real \<Rightarrow> nat \<Rightarrow> timed_vars \<Rightarrow> bool" where
  "behavConDiag3 bl cl t0 t k h = behavConDiag cl t0 t (exe_diag_tilltime bl cl h k) 
...
definition behavConDiag2 :: "block list \<Rightarrow> block list \<Rightarrow> real \<Rightarrow> real \<Rightarrow> nat \<Rightarrow> timed_vars \<Rightarrow> bool" where
  "behavConDiag2 bl cl t0 t k h = behavConSingle cl t0 t (exe_diag_tilltime bl cl h k) 
  (exe_diag_tilltime bl cl h (k-1))"


lemma existence_of_diag_pre:"
dl = (getCalBlks cl) \<Longrightarrow> el = (rev (sortDiag (bl@(getCalBlks cl)))) \<Longrightarrow>
...
\<forall>t::nat < k. behavConDiag3 bl cl t 1 k h \<and> 
...
proof(induction k)
  case 0
  then show ?case using existence_of_continuous_diag_single by simp
next
  case (Suc k)
  have 0: "besorted (rev el)"
    proof -
      ...
      show ?thesis using Suc(2,3) tmp2 tmp3 sort_is_sorted by simp
    qed
  ...
  proof(cases "t = k")
    ...
    then show ?thesis using 4 unfolding behavDisDiag_interval2.simps exe_diag_tilltime.simps behavConDiag3_def
      using existence_of_diag_interval'[of dl cl el bl k 1 "(exe_diag_tilltime bl cl h k)"] 
    ...
    next
      ...
      proof -
        ...
        have tmp1: "behavConDiag cl (real t) 1 (exe_diag_tilltime bl cl h k) (exe_diag_tilltime bl cl h (k - 1))"
          using Suc 1 tmp0 unfolding behavConDiag3_def by simp
     ...
     (exe_diag_tilltime bl cl h (k - 1))"
      ...
      have 5: "behavDisDiag_attime (rev (sortDiag (bl @ getCalBlks cl))) 
        ...
        using exe_diag_tilltime_lemma 0 Suc(6) by (metis Suc.prems(1) rev_rev_ident)
      then show ?thesis using 2 3 4 Suc(2) unfolding behavConDiag3_def by simp
    qed
  qed
qed
  done

lemma existence_pre_for_combination:"
dl = (getCalBlks cl) \<Longrightarrow> el = (rev (sortDiag (bl@(getCalBlks cl)))) \<Longrightarrow>
besorted2 (rev dl)  \<Longrightarrow> wf2 cl \<Longrightarrow>
loop_free (bl @ getCalBlks cl) \<Longrightarrow> wf1 el \<Longrightarrow> Outputs bl \<inter> Outputs cl = {} \<Longrightarrow>
\<forall>x. x \<notin> Outputs bl \<union> Outputs cl \<longrightarrow> (\<forall>t. h x t = h v 0) \<Longrightarrow>
ODE_cond' (updateIntegBlks cl (findIntegBlks cl)) h k = True \<Longrightarrow>

\<forall>t::nat < k. behavConDiag2 bl cl t 1 k h"
  apply clarify subgoal premises pre for t
  proof -
    have 1: "behavConDiag3 bl cl t 1 k h"
      using existence_of_diag_pre pre by auto
    have 2: "k = Suc(k-1)"
      using pre(10) by auto
    have 3: "ODE_cond (updateIntegBlks cl (findIntegBlks cl)) (exe_diag_tilltime bl cl h (k - 1)) (real t) 1"
      using pre(8) unfolding ODE_cond'_def using ODE_cond_def by presburger
    have 4: "checkODE (updateIntegBlks cl (findIntegBlks cl)) (exe_diag_tilltime bl cl h k)
     (exe_diag_tilltime bl cl h (k - 1)) (real t) 1"
      using 1 unfolding behavConDiag3_def behavConDiag.simps by simp
    have 5: "behavCalBlks (getCalBlks cl) (real t) 1 (exe_diag_tilltime bl cl h k)"
      using 1 unfolding behavConDiag3_def behavConDiag.simps by simp
    have 6: "\<forall>x\<in>Outputs cl.
       exe_diag_tilltime bl cl h k x (real t) = exe_diag_tilltime bl cl h (k - 1) x (real t)"
      apply clarify subgoal premises pre6 for x
    proof -
      have tmp1: "real t < (nat \<lfloor>real (k - 1) + 1\<rfloor>)"
        using pre(10) by force
      have tmp2: "exeDisDiag_attime (rev (sortDiag (bl @ getCalBlks cl)))
   (exeContinuousDiag_interval cl
     (exeDisDiag_interval bl (exe_diag_tilltime bl cl h (k - 1)) (real (k - 1)) 1) (real (k - 1)) 1)
   (nat \<lfloor>real (k - 1) + 1\<rfloor>) x (real t)= (exeContinuousDiag_interval cl
     (exeDisDiag_interval bl (exe_diag_tilltime bl cl h (k - 1)) (real (k - 1)) 1) (real (k - 1)) 1)
      x (real t)"
        using pre6 tmp1 exeDisDiag_attime_lemma2 by auto
      show ?thesis using exe_diag_tilltime.simps(2)[of bl cl h "k-1"] unfolding exe_diag_interval.simps 
        using tmp2 by (smt (verit, best) "2" Suc_as_int Suc_eq_plus1 \<open>exe_diag_tilltime bl cl h (Suc (k - 1)) 
        = exe_diag_interval bl cl (exe_diag_tilltime bl cl h (k - 1)) (real (k - 1)) 1\<close> 
        add_diff_cancel_left' exe_diag_interval_lemma1 floor_diff_one floor_of_nat lessI 
        linorder_not_le nat_less_real_le of_nat_1 of_nat_add of_nat_less_iff tmp1)
    qed
    done
    have 7: "loop_free (getCalBlks cl)"
      using pre(2,3) loop_free_perm besorted2_loopfree Unique_rev getCalBlkswf perm_rev wf2_def by blast
    have 8: "\<forall>x\<in>Inputs cl - Outputs cl.
       \<forall>tt. real t < tt \<and> tt < real t + 1 \<longrightarrow>
            exe_diag_tilltime bl cl h k x tt = exe_diag_tilltime bl cl h (k - 1) x (real t)"
      apply clarify subgoal premises pre8 for x tt
      proof -
          show ?thesis using exe_diag_tilltime_eq3 pre(3,4,7) pre8 by (simp add: pre(10))
      qed
      done
    show ?thesis unfolding behavConDiag2_def behavConSingle.simps  
      using checkODE_forSingle[where bl = cl and h = "(exe_diag_tilltime bl cl h k)" and 
          ?h0.0 = "(exe_diag_tilltime bl cl h (k - 1))" and ?t0.0 = t and t = 1 and
          b = "(updateIntegBlks cl (findIntegBlks cl))"] 3 4 5 6 7 8 pre(3) zero_less_one by blast
  qed
  done

\<comment> \<open>note: existence!!!\<close>
...
behavDisDiag_attime el (t+1) (exe_diag_tilltime bl cl h k) \<and>
behavDisDiag_attime el 0 (exe_diag_tilltime bl cl h k)"
  using existence_pre_for_combination existence_of_diag_pre by (metis (full_types))
...
\<comment> \<open>note: uniqueness!!!\<close>
lemma uniqueness_of_diag: "cl <~~> cl' \<Longrightarrow> dl = getCalBlks cl \<Longrightarrow> dl' = (getCalBlks cl') 
\<Longrightarrow> besorted2 (rev dl) \<Longrightarrow> wf2 cl \<Longrightarrow> besorted2 (rev dl') \<Longrightarrow>
el = rev (sortDiag (bl @ getCalBlks cl)) \<Longrightarrow> 
el' = rev (sortDiag (bl' @ getCalBlks cl')) \<Longrightarrow> 
...
section \<open>definition and execution for subsystem\<close>

text \<open>general subsystem\<close>
\<comment> \<open>discrete blocks, continuous blocks, subsystem list
EnableSub: discrete blocks, Enable var, subsystem list 
TriggerSub: discrete blocks, Enable var, trigger type, subsystem list
(enabled and triggered subsystems has no continuous blocks) \<close>
datatype subsystem = Subsystem "DisBlk list" "ConBlk list" "subsystem list"
  | EnableSub "DisBlk list"  var "subsystem list"
  | TriggerSub "DisBlk list" var nat "subsystem list"

(*type: 1 \<longrightarrow> rising 2 \<longrightarrow> falling 3 \<longrightarrow> either*)


fun getDisBlks :: "subsystem \<Rightarrow> DisBlk list" where
  "getDisBlks (Subsystem a _ _) = a" |
  "getDisBlks (EnableSub a _ _) = a" |
  "getDisBlks (TriggerSub a _ _ _ ) = a"

fun getConBlks :: "subsystem \<Rightarrow> ConBlk list" where
  "getConBlks (Subsystem _ b _) = b" |
  "getConBlks (EnableSub _ _ _) = []" |
  "getConBlks (TriggerSub _ _ _ _ ) = []"

fun getControl :: "subsystem \<Rightarrow> var" where
  "getControl (Subsystem _ _ _) = undefined" |
  "getControl (EnableSub _ c _) = c" |
  "getControl (TriggerSub _ c _ _) = c"

(*0 for blocks in a enabled subsystems, otherwise for blocks in a triggered subsystems 
and 0 otherwise
b \<Rightarrow> (CHR ''a'', 0) indicates that block b is controlled by ''a'' > 0;
b \<Rightarrow> (CHR ''a'', otherwise) indicates that block b is controlled by ''a'' zero-cross*)
type_synonym block_cond = "block \<Rightarrow> var list \<times> nat list"

inductive getBlockCond :: "subsystem list \<Rightarrow> block_cond \<Rightarrow> block_cond \<Rightarrow> bool" 
  and getAllBlocksInSub :: "subsystem list \<Rightarrow> block list \<Rightarrow> bool" where 
  "getBlockCond [] bcond bcond" | 
  "getBlockCond (subs@c) bcond1 bcond2 \<Longrightarrow> 
  getBlockCond ((Subsystem a b c)#subs) bcond1 bcond2" |
  "getBlockCond (subs@c) bcond1 bcond2 \<Longrightarrow>
  getAllBlocksInSub [(EnableSub a b c)] bl \<Longrightarrow>
  getBlockCond ((EnableSub a b c)#subs) bcond1 (\<lambda>x. (if x \<in> set bl then 
  (b#(fst (bcond2 x)), 0#snd (bcond2 x)) else bcond2 x))" |
  "getBlockCond (subs@c) bcond1 bcond2 \<Longrightarrow> 
  getAllBlocksInSub [(TriggerSub a b n c)] bl \<Longrightarrow>
  getBlockCond ((TriggerSub a b n c)#subs) bcond1 (\<lambda>x. (if x \<in> set bl then 
  (b#(fst (bcond2 x)), n#snd (bcond2 x)) else bcond2 x))" |
  "getAllBlocksInSub [] []" |
  "getAllBlocksInSub (subs@c) bl \<Longrightarrow>
   getAllBlocksInSub ((Subsystem a b c)#subs) (a@bl)" |
  "getAllBlocksInSub (subs@c) bl \<Longrightarrow>
   getAllBlocksInSub ((EnableSub a b c)#subs) (a@bl)" |
  "getAllBlocksInSub (subs@c) bl \<Longrightarrow>
   getAllBlocksInSub ((TriggerSub a b n c)#subs) (a@bl)"


fun TriggerCond :: "var \<Rightarrow> nat \<Rightarrow> timed_vars \<Rightarrow> real \<Rightarrow> bool" where
  "TriggerCond v n h t = (if n = 1 then (h v t = 0 \<and> (\<exists>t1 > 0.(\<forall>t2 \<le> t1. h v (t+t2) < 0))) else
  if n = 2 then (h v t = 0 \<and> (\<exists>t1 > 0.(\<forall>t2 \<le> t1. h v (t+t2) > 0))) else (if n = 3 then 
  (h v t = 0 \<and> (\<exists>t1 > 0.(\<forall>t2 \<le> t1. h v (t+t2) \<noteq> 0 \<or> h v (t-t2) \<noteq> 0))) else True))"


fun getBlockStatus :: "var list \<times> nat list \<Rightarrow> timed_vars \<Rightarrow> real \<Rightarrow> bool" where
  "getBlockStatus ([], nl) h t = True" |
  "getBlockStatus (vl, []) h t = True" |
  "getBlockStatus ((v#vl), (n#nl)) h t = ((if n = 0 then (h v t > 0) else if n < 4 then 
  TriggerCond v n h t else True) \<and> (getBlockStatus (vl, nl) h t))"

definition getBlockActiveSet :: "block \<Rightarrow> block_cond \<Rightarrow> timed_vars \<Rightarrow> real \<Rightarrow> real \<Rightarrow> real set" where
  "getBlockActiveSet b bcond h t0 t = {x. (x > t0 \<and> x \<le> t0 + t \<and> getBlockStatus (bcond b) h x)}"

fun conditionalExecuteDis :: "DisBlk list \<Rightarrow> timed_vars \<Rightarrow> block_cond \<Rightarrow> nat \<Rightarrow> timed_vars" where
  "conditionalExecuteDis [] h bcond t = h" | 
  "conditionalExecuteDis (b#bl) h bcond t = (if (getBlockStatus (bcond b) h t) \<and> 
  (\<exists>k. t = (get_sample_time b)*k \<or> (get_sample_time b) = 0)
   then (outupd_exe_atst (get_inputs b) (get_outputs b) (get_offsets b) (get_outupd b) 
  (conditionalExecuteDis bl h bcond t) t) else 
  (outupd_exe_notatst (get_inputs b) (get_outputs b) (get_outupd b) (get_sample_time b) 
  (conditionalExecuteDis bl h bcond t) t)) "

fun con_exe_diag_interval :: "DisBlk list \<Rightarrow> ConBlk list \<Rightarrow> 
  timed_vars \<Rightarrow> block_cond \<Rightarrow> real \<Rightarrow> real \<Rightarrow> timed_vars" where
  "con_exe_diag_interval bl cl h bcond t0 t =  (conditionalExecuteDis 
(rev (sortDiag (bl@((getCalBlks cl))))) (exeContinuousDiag_interval cl (exeDisDiag_interval bl h t0 t) 
  t0 t) bcond (nat (floor (t0+t))))"

fun con_exe_diag_tilltime :: "DisBlk list \<Rightarrow> ConBlk list 
    \<Rightarrow> timed_vars \<Rightarrow> block_cond \<Rightarrow> nat \<Rightarrow> timed_vars" where
  "con_exe_diag_tilltime bl cl h bcond 0 = 
    conditionalExecuteDis (rev (sortDiag (bl@((getCalBlks cl))))) h bcond 0" |
  "con_exe_diag_tilltime bl cl h bcond (Suc k) = con_exe_diag_interval bl cl 
  (con_exe_diag_tilltime bl cl h bcond k) bcond k 1"
...
inductive exe_diag_with_subsystem_interval :: "DisBlk list \<Rightarrow> ConBlk list \<Rightarrow> 
  subsystem list \<Rightarrow> timed_vars \<Rightarrow> block_cond \<Rightarrow> real \<Rightarrow> real \<Rightarrow> timed_vars \<Rightarrow> bool" where
  "exe_diag_with_subsystem_interval bl cl [] h bcond t0 t (con_exe_diag_interval 
    (sortDiag bl) ((sortDiag (getCalBlks cl))@(findIntegBlks cl)) h bcond t0 t)" |

  "exe_diag_with_subsystem_interval (bl@(getBlockAttime t0 t (getDisBlks sub)))
  (cl@(getConBlks sub)) (subs@c) h bcond t0 t h1 \<Longrightarrow>
  exe_diag_with_subsystem_interval bl cl ((Subsystem a b c)#subs) h bcond t0 t h1"
