...
lemma upd_upd_simp':
  "X\<noteq>Y \<Longrightarrow> upd (upd s X v1) Y v2 = upd (upd s Y v2) X v1"
  apply (cases s) by auto

lemma rpart_upd_simp1 [simp]:
  "rpart (upd s X v) X = v"
  apply (cases s) by auto

lemma rpart_upd_simp2 [simp]:
  "X\<noteq>Y \<Longrightarrow> rpart (upd s X v) Y = rpart s Y"
  apply (cases s) by auto
...
lemma rel_list_ll:
  assumes "length xs = length ys"
    and "\<And>i. i<length xs \<Longrightarrow> r (xs ! i) (ys ! i)"
  shows "rel_list r xs ys"
  using assms
  apply (induct "length xs" arbitrary: xs ys)
  subgoal 
    apply simp
    apply(rule rel_list.intros(1))
    done
  subgoal for n xs ys
    apply (cases xs)
     apply auto
    subgoal for x xss
     ...
     apply auto
      subgoal for y yss
        apply(rule rel_list.intros(2))
         apply fastforce
        by (metis Ex_less_Suc less_trans_Suc nth_Cons_Suc)
      done
    done
  done
...
lemma cond_prop:
"(IFA b THEN Q1 ELSE Q2 FI) = ((!\<^sub>a[b] \<and>\<^sub>a Q1) \<or>\<^sub>a (!\<^sub>a[(\<lambda> s. \<not>b s)] \<and>\<^sub>a Q2))"
  apply rule +
  unfolding cond_assn2_def pure_assn_def conj_assn_def disj_assn_def
   apply metis+
  done
...
\<close>
lemma subst_exists:
  "(\<exists>\<^sub>an. P n) {{var := e}} = (\<exists>\<^sub>an. P n {{var := e}})"
  apply (rule ext) apply (rule ext) apply (rule ext)
  subgoal for s0 s tr
    apply (rule iffI)
    subgoal unfolding exists_assn_def subst_assn2_def
      by auto
    subgoal unfolding exists_assn_def subst_assn2_def
      by auto
    done
...
definition spec_of_gen :: "('a estate \<Rightarrow> bool) \<Rightarrow> 'a proc \<Rightarrow> 'a assn2 \<Rightarrow> bool" where
  "spec_of_gen P c Q \<longleftrightarrow> (\<forall>s0. P s0 \<longrightarrow> \<Turnstile> {\<lambda>s. s = s0} c {Q s0})"
