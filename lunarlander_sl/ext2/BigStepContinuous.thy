...
lemma interrupt_solInf_c_emp:
  "interrupt_solInf_c I [] = false_assn2"
  unfolding false_assn2_def
  apply rule+
   apply auto
  apply(elim interrupt_solInf_c.cases)
  by auto
...
lemma interrupt_sol_c_exists1:
"interrupt_sol_c I e (\<lambda>d. \<exists>\<^sub>an. P d n) cm = (\<exists>\<^sub>an. interrupt_sol_c I e (\<lambda>d. P d n) cm)"
  apply (rule ext)+
  subgoal for s0 s tr
    apply (rule iffI)
    subgoal unfolding exists_assn_def
      apply (induct rule: interrupt_sol_c.cases) apply auto
      subgoal for tra a b p n
      ...
      subgoal for n
        ...
        apply(rule interrupt_sol_c.intros(2))
        by auto
      subgoal for i ch Q v tra
        apply(rule exII)
        apply(rule interrupt_sol_c.intros(3))
          apply simp+
        done
      subgoal for i ch Q d v tra a b p
        apply(rule exII)
        apply(rule interrupt_sol_c.intros(4))
          apply simp+
        done
      subgoal for i ch Q v tra
        apply(rule exII)
        apply(rule interrupt_sol_c.intros(5))
          apply simp+
        done
      subgoal for i ch Q d v tra a b p
        apply(rule exII)
        apply(rule interrupt_sol_c.intros(6))
          apply simp+
        done
      done
    subgoal unfolding exists_assn_def
...
lemma list_assist:
  "i\<noteq> length a \<Longrightarrow> (a@[x]@b) ! i = (a@[y]@b) ! i"
  apply(cases "i<length a")
  subgoal by (simp add: nth_append)
  subgoal by (metis cons_append list_update_length nth_list_update_neq)
  done

lemma int_exists2_assist:
  "rdy_of_comm_spec2 (cm1 @ InSpec2 ch (\<lambda>d v s0 s tr. \<exists>n. Q d v n s0 s tr) # cm2) =
         rdy_of_comm_spec2 (cm1 @ InSpec2 ch (\<lambda>d v. Q d v n) # cm2)"
    unfolding rdy_of_comm_spec2_def
        apply (rule rdy_info_of_list_cong)
        apply (rule rel_list_ll)
         apply auto
        subgoal for  i
          apply (cases "i<length cm1")
          subgoal apply simp 
            by (smt (verit, best) dual_order.irrefl list_update_length nth_list_update_neq)
          subgoal
            apply(cases "i=length cm1")
            subgoal by simp
...
lemma interrupt_sol_c_exists2:
  "interrupt_sol_c I e P (cm1@[InSpec2 ch (\<lambda>d v. \<exists>\<^sub>an. Q d v n)]@cm2) = (\<exists>\<^sub>an. interrupt_sol_c I e P (cm1@[InSpec2 ch (\<lambda>d v. Q d v n)]@cm2))"
  apply (rule ext)+
  subgoal for s0 s tr
    apply (rule iffI)
    subgoal unfolding exists_assn_def
      ...
      subgoal for tra a b p
        ...
        apply(rule interrupt_sol_c.intros(1))
           apply auto
        by(rule int_exists2_assist)
      subgoal 
        apply(rule exII)
        apply(rule interrupt_sol_c.intros(2))
           by auto
         subgoal for i cha Qa v tra
           apply(cases "i \<noteq> length cm1")
           subgoal 
             apply(rule exII)
             apply(rule interrupt_sol_c.intros(3)[of i _ cha Qa])
               apply auto
             by (metis (no_types, lifting) list_update_length nth_list_update_neq)
           apply auto
           subgoal for n
             apply(rule exI[where x=n])
             apply(rule interrupt_sol_c.intros(3)[of i _ cha "(\<lambda>d v. Q d v n)"])
             by auto
           done
         subgoal for i cha Qa d v tra a b p
           apply(cases "i \<noteq> length cm1")
           subgoal 
             apply(rule exII)
             apply(rule interrupt_sol_c.intros(4)[of i _ cha Qa])
                   apply auto
              prefer 2
             apply (rule int_exists2_assist)
             by (metis (no_types, lifting) list_update_length nth_list_update_neq)
           apply auto
           subgoal for n
             apply(rule exI[where x=n])
             apply(rule interrupt_sol_c.intros(4)[of i _ cha "(\<lambda>d v. Q d v n)"])
                   apply auto
             by (rule int_exists2_assist)
           done
         subgoal for i cha Qa v tra
           ...
           subgoal 
             apply(rule exII)
             apply(rule interrupt_sol_c.intros(5)[of i _ cha Qa])
               apply auto
             by (metis (no_types, lifting) list_update_length nth_list_update_neq)
           by auto
         subgoal for i cha Qa d v tra a b p
           ...
           subgoal 
             apply(rule exII)
             apply(rule interrupt_sol_c.intros(6)[of i _ cha Qa])
                   apply auto
              prefer 2
             apply (rule int_exists2_assist)
             by (metis (no_types, lifting) list_update_length nth_list_update_neq)
       ...
       subgoal unfolding exists_assn_def
         apply auto subgoal for n
           ...
           subgoal for tra a b p
             apply(rule interrupt_sol_c.intros(1))
             apply auto
             by (rule int_exists2_assist[symmetric])
           subgoal 
             apply(rule interrupt_sol_c.intros(2))
             by auto
           subgoal for i cha Qa v tra
             ...
             subgoal 
               ...
               by (metis (no_types, lifting) list_update_length nth_list_update_neq)
             subgoal apply auto
               apply(rule interrupt_sol_c.intros(3)[of i _ cha])
                 apply simp+
               apply(rule exI[where x=n])
               by auto
             done
           subgoal for i cha Qa d v tra a b p
             ...
             subgoal 
               apply(rule interrupt_sol_c.intros(4)[of i _ cha Qa])
                     apply auto
                prefer 2
               apply (rule int_exists2_assist[symmetric])
               by (metis (no_types, lifting) list_update_length nth_list_update_neq)
             subgoal apply auto
               apply(rule interrupt_sol_c.intros(4)[of i _ cha])
                 apply simp+
               apply(rule exI[where x=n])
                 apply auto
               by (rule int_exists2_assist[symmetric])
...
lemma int_exists3_assist:
  "rdy_of_comm_spec2 (cm1 @ OutSpec2 ch (\<lambda>d v s0 s tr. \<exists>n. Q d v n s0 s tr) # cm2) =
         rdy_of_comm_spec2 (cm1 @ OutSpec2 ch (\<lambda>d v. Q d v n) # cm2)"
    unfolding rdy_of_comm_spec2_def
        apply (rule rdy_info_of_list_cong)
        apply (rule rel_list_ll)
         apply auto
...
lemma interrupt_sol_c_exists3:
  "interrupt_sol_c I e P (cm1@[OutSpec2 ch (\<lambda>d v. \<exists>\<^sub>an. Q d v n)]@cm2) = (\<exists>\<^sub>an. interrupt_sol_c I e P (cm1@[OutSpec2 ch (\<lambda>d v. Q d v n)]@cm2))"
  apply (rule ext)+
  subgoal for s0 s tr
    apply (rule iffI)
    subgoal unfolding exists_assn_def
      ...
      subgoal 
        ...
        apply(rule interrupt_sol_c.intros(2))
         ...
         subgoal for i cha Qa v tra
           ...
           subgoal for n
             apply(rule exI[where x=n])
             apply(rule interrupt_sol_c.intros(5)[of i _ cha "(\<lambda>d v. Q d v n)"])
             by auto
           done
         subgoal for i cha Qa d v tra a b p
           apply(cases "i \<noteq> length cm1")
           ...
           subgoal for n
             apply(rule exI[where x=n])
             apply(rule interrupt_sol_c.intros(6)[of i _ cha "(\<lambda>d v. Q d v n)"])
                   apply auto
             by (rule int_exists3_assist)
           done
         done
       subgoal unfolding exists_assn_def
         apply auto subgoal for n
           ...
           subgoal for tra a b p
             apply(rule interrupt_sol_c.intros(1))
             apply auto
             by (rule int_exists3_assist[symmetric])
           subgoal 
             apply(rule interrupt_sol_c.intros(2))
             by auto
           subgoal for i cha Qa v tra
           ...
           subgoal for i cha Qa d v tra a b p
             ...
             subgoal 
               apply(rule interrupt_sol_c.intros(4)[of i _ cha Qa])
                     apply auto
                prefer 2
               apply (rule int_exists3_assist[symmetric])
               by (metis (no_types, lifting) list_update_length nth_list_update_neq)
             by auto
           subgoal for i cha Qa v tra
             apply(cases "i\<noteq>length cm1")
             subgoal 
               ...
               by (metis (no_types, lifting) list_update_length nth_list_update_neq)
             subgoal apply auto
               apply(rule interrupt_sol_c.intros(5)[of i _ cha])
                 apply simp+
               apply(rule exI[where x=n])
               by auto
             done
           subgoal for i cha Qa d v tra a b p
             ...
             subgoal 
               ...
               by (metis (no_types, lifting) list_update_length nth_list_update_neq)
             subgoal apply auto
               apply(rule interrupt_sol_c.intros(6)[of i _ cha])
                 apply simp+
               apply(rule exI[where x=n])
                 apply auto
               by (rule int_exists3_assist[symmetric])
...
lemma inv_ass1:     
  assumes "paramODEsol ode b p d "
   and "f s0 \<Longrightarrow> \<forall>t. t \<ge> 0 \<and> t \<le> d s0 \<longrightarrow> Inv (updr s0 (p s0 t))"  
   and "local_lipschitz {- 1<..} UNIV (\<lambda>(t::real) v. ODE2Vec ode (vec2state v))"
 shows "ode_c ode b P s0 \<Longrightarrow>\<^sub>A ((!\<^sub>a[(\<lambda> s. d s0 \<le> 0)] \<and>\<^sub>a P) \<or>\<^sub>a (!\<^sub>a[(\<lambda> s. f s \<and> d s > 0)] \<and>\<^sub>a wait_c (\<lambda> _ _ s. Inv s) d (\<lambda>d'. \<exists>\<^sub>ass. (!\<^sub>a[Inv] \<and>\<^sub>a P){{(\<lambda> _ . ss)}}\<^sub>r )) \<or>\<^sub>a (!\<^sub>a[(\<lambda> s. \<not> f s \<and> d s > 0)] \<and>\<^sub>a true_assn2)) s0"
  apply(rule entails_trans)
   apply(rule ode_c_unique)
  using assms
    apply simp+
  unfolding entails_def pure_assn_def conj_assn_def disj_assn_def exists_assn_def substr_assn2_def
  apply clarify
  subgoal for s tr
    apply(cases "d s0\<le>0")
    subgoal apply (elim wait_c.cases)
       apply auto
      using assms(1) unfolding paramODEsol_def by auto
    apply(cases"\<not> f s0")
    subgoal
      apply auto unfolding true_assn2.simps by auto
    subgoal
     apply simp
     apply (elim wait_c.cases)
      ...
      subgoal
        apply(rule wait_c.intros(1))
        apply auto
         apply(rule exI[where x= "(p s0 (d s0))"])
        using assms(2)
        by auto
      done
    done
...
definition ODEinv :: "ODE \<Rightarrow> 'a eform \<Rightarrow> 'a eexp \<Rightarrow>'a eform \<Rightarrow> 'a eform  \<Rightarrow> bool" where
  "ODEinv ode b d Init Inv \<longleftrightarrow>
    (\<forall>s . 
        (if b s then
            d s > 0 \<and> (\<forall>p dd. p 0 = rpart s \<and>
            ODEsol ode (p) (dd) \<and> (\<forall>t. t \<ge> 0 \<and> t < dd \<longrightarrow> b (updr s (p t))) \<and> \<not> b (updr s (p dd)) \<longrightarrow> 
            dd = d s \<and> (Init s \<longrightarrow> (\<forall>t. t \<ge> 0 \<and> t \<le> d s \<longrightarrow> Inv (updr s (p t))))) 
         else
            \<not> d s > 0))"


lemma ODEinvD:
  assumes "ODEinv ode b d Init Inv"
    and "b s "
  shows "d s > 0"
  using assms unfolding ODEinv_def by auto

lemma ODEinvD1:
  assumes "ODEinv ode b d Init Inv"
    and "\<not> b s "
  shows "d s \<le> 0"
  using assms unfolding ODEinv_def 
  by (metis linorder_le_less_linear)

lemma ODEinvD2:
  assumes "ODEinv ode b d Init Inv"
    and "b s \<and> Init s"
    and "ODEsol ode (p) (dd) \<and> p 0 = rpart s"
    and "\<forall>t.  t \<ge> 0 \<and> t < dd \<longrightarrow> b (updr s (p t))"
    and "\<not> b (updr s (p dd))"
  shows "dd = d s" "t \<ge> 0 \<and> t \<le> d s \<longrightarrow> Inv (updr s (p t))"
  using assms unfolding ODEinv_def by auto

lemma inv_ass2:     
  assumes "ODEinv ode b d f Inv"
  shows "ode_c ode b P s0 \<Longrightarrow>\<^sub>A ((!\<^sub>a[(\<lambda> s. d s0 \<le> 0)] \<and>\<^sub>a P) 
          \<or>\<^sub>a (!\<^sub>a[(\<lambda> s. f s \<and> d s > 0)] \<and>\<^sub>a wait_c (\<lambda> _ _ s. Inv s) d (\<lambda>d'. \<exists>\<^sub>ass. (!\<^sub>a[Inv] \<and>\<^sub>a P){{(\<lambda> _ . ss)}}\<^sub>r )) 
          \<or>\<^sub>a (!\<^sub>a[(\<lambda> s. \<not> f s \<and> d s > 0)] \<and>\<^sub>a true_assn2)) s0"
  unfolding entails_def
  apply auto
  subgoal for s tr
    apply (auto simp add: ode_c.simps)
    subgoal unfolding pure_assn_def conj_assn_def disj_assn_def
      using ODEinvD1[OF assms]
      by auto
    subgoal for dd p trr
      apply(subgoal_tac "d s0 > 0")
       prefer 2
       apply (auto simp add: ODEinvD[OF assms])
      apply(cases "f s0")
      subgoal unfolding pure_assn_def conj_assn_def disj_assn_def
        apply simp
        using ODEinvD2[of ode b d f Inv s0 p dd] assms
        apply simp
        apply(subgoal_tac "b s0")
         apply auto
        apply (rule wait_c.intros(1))
          apply auto unfolding exists_assn_def substr_assn2_def
        apply(rule exI[where x= "p (d s0)"])
        by auto
      subgoal unfolding pure_assn_def conj_assn_def disj_assn_def
        apply simp
        apply rule
...
lemma inv_ass3:     
  assumes "ODEinv ode b d f Inv"
     and "d s0 > 0 \<Longrightarrow> f s0"
  shows "ode_c ode b P s0 \<Longrightarrow>\<^sub>A ((!\<^sub>a[(\<lambda> s. d s0 \<le> 0)] \<and>\<^sub>a P) 
          \<or>\<^sub>a (!\<^sub>a[(\<lambda> s. f s \<and> d s > 0)] \<and>\<^sub>a wait_c (\<lambda> _ _ s. Inv s) d (\<lambda>d'. \<exists>\<^sub>ass. (!\<^sub>a[Inv] \<and>\<^sub>a P){{(\<lambda> _ . ss)}}\<^sub>r ))) s0"
  apply(rule entails_trans)
   apply(rule inv_ass2[OF assms(1)])
  unfolding entails_def pure_assn_def conj_assn_def disj_assn_def
  using assms(2) by auto
