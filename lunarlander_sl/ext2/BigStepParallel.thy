...
lemma updg_simps1 [simp]:
  "the ((updg gs pn x v) pn) = upd (the (gs pn)) x v"
  unfolding updg_def
  by auto

lemma updg_simps2 [simp]:
  "pm \<noteq> pn \<Longrightarrow> the ((updg gs pn x v) pm) = (the (gs pm))"
  unfolding updg_def
  by auto
...
lemma valg_State_simp [simp]:
  "valg (State pn s) pn var = val s var"
  unfolding valg_def by auto

lemma val_restrict [simp]:
  "p\<in>pn \<Longrightarrow> val (the (restrict_state pn s0 p)) V = val (the (s0 p)) V"
  unfolding restrict_state_def
  by auto
...
text \<open>Global version of path invariant\<close>
type_synonym 'a gpinv = "real \<Rightarrow> 'a gstate \<Rightarrow> bool"

text \<open>Global version of path invariant, parameterized by starting state\<close>
type_synonym 'a gpinv2 = "'a gstate \<Rightarrow> 'a gpinv"
...
definition cond_gassn2 :: "('a gstate \<Rightarrow> bool) \<Rightarrow> 'a gassn2 \<Rightarrow> 'a gassn2 \<Rightarrow> 'a gassn2"
  ("IFG _ THEN _ ELSE _ FI" [95,95,94] 93) where
  "IFG b THEN Q1 ELSE Q2 FI = (\<lambda>s0 s tr. if b s0 then Q1 s0 s tr else Q2 s0 s tr)"
...
inductive wait_cg :: "'a gpinv2 \<Rightarrow> pname \<Rightarrow> 'a eexp \<Rightarrow> (real \<Rightarrow> 'a gassn2) \<Rightarrow> 'a gassn2" where
...
text \<open>
  Another generalization of wait_in_c. This version combines
  waiting for input with a time limit.
\<close>
inductive wait_in_cg_alt :: "'a gpinv2 \<Rightarrow> cname \<Rightarrow> pname \<Rightarrow> 'a eexp \<Rightarrow> (real \<Rightarrow> real \<Rightarrow> 'a gassn2) \<Rightarrow> (real \<Rightarrow> 'a gassn2) \<Rightarrow> 'a gassn2" where
...
definition wait_in_c0 :: "'a pinv2 \<Rightarrow> cname \<Rightarrow> (real \<Rightarrow> 'a assn2) \<Rightarrow> 'a assn2" where
  "wait_in_c0 I ch P = wait_in_c I ch (\<lambda>d v. IFA (\<lambda>s. d = 0) THEN P v ELSE true_assn2 FI)"

definition wait_in_cg0 :: "'a gpinv2  \<Rightarrow> cname \<Rightarrow> pname set \<Rightarrow> (real \<Rightarrow> 'a gassn2) \<Rightarrow> 'a gassn2" where
  "wait_in_cg0 I ch pns P = wait_in_cg I ch (\<lambda>d v. IFG (\<lambda>s. d = 0) THEN P v ELSE true_gassn pns FI)"

subsubsection \<open>More definition on paths\<close>
...
inductive single_inv :: "pname \<Rightarrow> 'a pinv2 \<Rightarrow> 'a gpinv2" where
...
inductive merge_inv :: "'a gpinv2 \<Rightarrow> 'a gpinv2 \<Rightarrow> 'a gpinv2" where
...
inductive delay_inv :: "real \<Rightarrow> 'a gpinv2 \<Rightarrow> 'a gpinv2" where
  "I s0 (t + d) s \<Longrightarrow> delay_inv d I s0 t s"

lemma delay_inv_0 [simp]:
  "delay_inv 0 I = I"
  apply (rule ext) apply (rule ext) apply (rule ext)
  subgoal for s0 t s
    apply (rule iffI)
    subgoal apply (elim delay_inv.cases) by auto
    subgoal apply (intro delay_inv.intros) by auto
    done
  done

fun id_inv :: "pname set \<Rightarrow> 'a gpinv2" where
  "id_inv pns gs0 t gs = (proc_set gs0 = pns \<and> gs0 = gs)"

lemma single_id_inv_simp [simp]:
  "single_inv pn single_id_inv = id_inv {pn}"
  apply (rule ext) apply (rule ext) apply (rule ext)
  subgoal for s0 t s
    apply (rule iffI)
    subgoal apply (elim single_inv.cases) by auto
    subgoal unfolding id_inv.simps apply clarify
      by (metis proc_set_single_elim single_id_inv.simps single_inv.intros)
    done
  done

lemma merge_inv_elim:
  assumes "merge_inv I1 I2 s0 t s"
...
lemma single_assn_wait_in [single_assn_simps]:
  "single_assn pn (wait_in_c I ch1 P) = wait_in_cg (single_inv pn I) ch1 (\<lambda>d v. single_assn pn (P d v))"
  apply (rule ext) apply (rule ext) apply (rule ext)
  subgoal for s0 s tr
    apply (rule iffI)
    subgoal apply (elim single_assn.cases) apply auto
      subgoal for s0' s' tr'
        ...
        apply (intro wait_in_cg.intros single_assn.intros)
        by (auto intro: single_inv.intros)
      done
...
lemma single_assn_wait_out [single_assn_simps]:
  "single_assn pn (wait_out_c I ch1 P) = wait_out_cg (single_inv pn I) ch1 (\<lambda>d v. single_assn pn (P d v))"
  apply (rule ext) apply (rule ext) apply (rule ext)
  subgoal for s0 s tr
    apply (rule iffI)
    subgoal apply (elim single_assn.cases) apply auto
...
lemma single_assn_wait_out_v [single_assn_simps]:
  "single_assn pn (wait_out_cv I ch1 e P) =
   wait_out_cgv (single_inv pn I) ch1 {pn} pn e (\<lambda>d. single_assn pn (P d))"
...
lemma single_assn_cond [single_assn_simps]:
  "single_assn pn (IFA b THEN a1 ELSE a2 FI) =
    (IFG (\<lambda> s0. b (the (s0 pn))) THEN single_assn pn a1 ELSE single_assn pn a2 FI)"
  apply (rule ext) apply (rule ext) apply (rule ext)
  subgoal for s0 s tr
    apply (rule iffI)
    subgoal
...
lemma single_assn_wait_in0 [single_assn_simps]:
  "single_assn pn (wait_in_c0 I ch P) =
   wait_in_cg0 (single_inv pn I) ch {pn} (\<lambda>v. single_assn pn (P v))"
...
lemma proc_set_path_id [intro]:
  "proc_set_path pns (id_inv pns)"
  unfolding proc_set_path_def id_inv.simps by auto

lemma proc_set_path_delay [intro!]:
...
lemma proc_set_wait_in_cg [intro!]:
  ...
  shows "proc_set_gassn pns (wait_in_cg I ch P)"
  unfolding proc_set_gassn_def apply clarify
  subgoal for gs0 gs tr
    apply (elim wait_in_cg.cases) apply clarify
    subgoal for _ v s0 s tr' I ch'
      using assms(1)[of 0 v] unfolding proc_set_gassn_def apply auto
      apply (rule proc_set_trace.intros) by blast
    subgoal for d P' v s0 s I tr' ch'
      using assms(1)[of d v] unfolding proc_set_gassn_def apply auto
      apply (rule proc_set_trace.intros)
      using assms(2)[unfolded proc_set_path_def]
       prefer 2 apply (meson atLeastAtMost_iff atLeast_iff)
      apply (rule proc_set_trace.intros) by blast
    done
  done
...
lemma proc_set_wait_out_cg [intro!]:
  assumes "\<And>d v. proc_set_gassn pns (P d v)"
    and "proc_set_path pns I"
  shows "proc_set_gassn pns (wait_out_cg I ch P)"
  unfolding proc_set_gassn_def apply clarify
  subgoal for gs0 gs tr
...
lemma proc_set_cond_gassn [intro!]:
  ...
  shows "proc_set_gassn pns (IFG b THEN P ELSE Q FI)"
  unfolding proc_set_gassn_def apply clarify
  subgoal for gs0 gs tr
...
lemma proc_set_updeg_subst [intro!]:
  assumes "proc_set_gassn pns P"
    and "pn \<in> pns"
  shows "proc_set_gassn pns (P {{ f }}\<^sub>g at pn)"
  unfolding proc_set_gassn_def apply clarify
  unfolding updeg_assn2_def using assms unfolding proc_set_gassn_def
  by (metis (mono_tags, lifting) mem_Collect_eq proc_set_def updeg_proc_set)
...
lemma disj_gassn_disjI1:
  assumes "P1 s0 s tr"
    shows "(P1 \<or>\<^sub>g P2) s0 s tr"
  using assms unfolding entails_g_def disj_gassn_def by auto

lemma disj_gassn_disjI2:
  assumes "P2 s0 s tr"
    shows "(P1 \<or>\<^sub>g P2) s0 s tr"
  using assms unfolding entails_g_def disj_gassn_def by auto
...
lemma cond_gassn_true:
  "b \<Longrightarrow> (IFG (\<lambda>_. b) THEN P ELSE Q FI = P)"
  unfolding cond_gassn2_def by auto

lemma cond_gassn_false:
  "\<not>b \<Longrightarrow> (IFG (\<lambda>_. b) THEN P ELSE Q FI = Q)"
  unfolding cond_gassn2_def by auto
...
lemma sync_gassn_disj':
  assumes "sync_gassn chs pns1 pns2 P Q1 gs0 \<Longrightarrow>\<^sub>G R"
    and "sync_gassn chs pns1 pns2 P Q2 gs0 \<Longrightarrow>\<^sub>G R"
  shows "sync_gassn chs pns1 pns2 P (Q1 \<or>\<^sub>g Q2) gs0 \<Longrightarrow>\<^sub>G R"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    using assms unfolding disj_gassn_def entails_g_def
...
lemma sync_gassn_disj2':
  assumes "sync_gassn chs pns1 pns2 P Q1 gs0 \<Longrightarrow>\<^sub>G R1 gs1"
    and "sync_gassn chs pns1 pns2 P Q2 gs0 \<Longrightarrow>\<^sub>G R2 gs1"
  shows "sync_gassn chs pns1 pns2 P (Q1 \<or>\<^sub>g Q2) gs0 \<Longrightarrow>\<^sub>G (R1 \<or>\<^sub>g R2) gs1"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    using assms unfolding disj_gassn_def entails_g_def
...
lemma sync_gassn_disj2'_s:
  assumes "sync_gassn chs pns1 pns2 P Q1 gs0 \<Longrightarrow>\<^sub>G R1 gs0"
    and "sync_gassn chs pns1 pns2 P Q2 gs0 \<Longrightarrow>\<^sub>G R2 gs0"
  shows "sync_gassn chs pns1 pns2 P (Q1 \<or>\<^sub>g Q2) gs0 \<Longrightarrow>\<^sub>G (R1 \<or>\<^sub>g R2) gs0"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    using assms unfolding disj_gassn_def entails_g_def
...
lemma sync_gassn_ifg_left:
  assumes
    "b (restrict_state pns1 s0) \<Longrightarrow> sync_gassn chs pns1 pns2 P1 Q s0 \<Longrightarrow>\<^sub>G R s0"
    "\<not>b (restrict_state pns1 s0) \<Longrightarrow> sync_gassn chs pns1 pns2 P2 Q s0 \<Longrightarrow>\<^sub>G R s0"
  shows
    "sync_gassn chs pns1 pns2 (IFG b THEN P1 ELSE P2 FI) Q s0 \<Longrightarrow>\<^sub>G R s0"
  ...
  subgoal for s tr
    ...
    proof (cases "b s11")
      ...
      show ?thesis
      proof -
        have "b (restrict_state pns1 s0)"
          unfolding pre(1) 
          by (auto simp add: True restrict_state_merge1 pre(7))
    ...
    next
      ...
      show ?thesis
      proof -
        have "\<not>b (restrict_state pns1 s0)"
          unfolding pre(1) 
          by (simp add: False restrict_state_merge1 pre(7))
...
lemma cond_gassn_mono:
  ...
  shows "(IFG b THEN P1 ELSE Q1 FI) s0 \<Longrightarrow>\<^sub>G (IFG b THEN P2 ELSE Q2 FI) s0"
...
lemma wait_out_cg_mono:
  ...
  shows "wait_out_cg I ch P s0 \<Longrightarrow>\<^sub>G wait_out_cg I ch Q s0"
  apply (auto simp add: entails_g_def)
  subgoal for s tr
...
lemma wait_in_cg_mono:
  assumes "\<And>d v. P d v s0 \<Longrightarrow>\<^sub>G Q d v s0"
  shows "wait_in_cg I ch P s0 \<Longrightarrow>\<^sub>G wait_in_cg I ch Q s0"
  apply (auto simp add: entails_g_def)
  subgoal for s tr
...
lemma wait_in_cg_alt_mono:
  ...
  shows "wait_in_cg_alt I ch pn e P1 Q1 s0 \<Longrightarrow>\<^sub>G wait_in_cg_alt I ch pn e P2 Q2 s0"
  apply (auto simp add: entails_g_def)
  subgoal for s tr
...
definition ParValid :: "('a gstate \<Rightarrow> bool) \<Rightarrow> 'a pproc \<Rightarrow> 'a gassn \<Rightarrow> bool" ("\<Turnstile>\<^sub>p ({(1_)}/ (_)/ {(1_)})" 50) where
...
lemma combine_blocks_waitE2':
  ...
  apply (induct rule: combine_blocks.cases, auto elim: WaitBlkP_eqE)
  subgoal premises pre for blks a b a' b' hist1 hist2 t
    apply (rule pre(7)) apply (rule WaitBlkP_eqI)
    using pre by (auto elim!: WaitBlkP_eqE)
  done
...
lemma sync_gassn_conj_pure_left':
  assumes "R (restrict_state pns1 s0) \<Longrightarrow> sync_gassn chs pns1 pns2 P1 P2 s0 \<Longrightarrow>\<^sub>G Q s0"
    and "pns1 \<inter> pns2 = {}"
  shows "sync_gassn chs pns1 pns2 (!\<^sub>g[R] at pn \<and>\<^sub>g P1) P2 s0 \<Longrightarrow>\<^sub>G Q s0"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply (auto simp add: conj_gassn_def pure_gassn_def)
    subgoal for s11 s12 s21 s22 tr1 tr2
      apply (rule entails_g_elim[where P="sync_gassn chs pns1 pns2 P1 P2 s0"]) apply auto
       apply (rule sync_gassn.intros) apply auto
      by (metis assms restrict_state_merge1)
    done
  done
...
lemma sync_gassn_in_outv':
  "ch \<in> chs \<Longrightarrow>
   ...
   pns1 \<inter> pns2 = {} \<Longrightarrow>
   sync_gassn chs pns1 pns2 (wait_in_cg I1 ch P) (wait_out_cgv I2 ch pns2 pn e Q) s0 \<Longrightarrow>\<^sub>G
...
lemma sync_gassn_out_in:
  "ch \<in> chs \<Longrightarrow>
   pns1 \<inter> pns2 = {} \<Longrightarrow>
   sync_gassn chs pns1 pns2 (wait_out_cg I1 ch Q) (wait_in_cg I2 ch P) s0 \<Longrightarrow>\<^sub>G
...
lemma sync_gassn_outv_in:
  "ch \<in> chs \<Longrightarrow>
   ...
   sync_gassn chs pns1 pns2 (wait_out_cgv I1 ch pns1 pn e Q) (wait_in_cg I2 ch P) s0 \<Longrightarrow>\<^sub>G
...
lemma sync_gassn_out_emp:
  "ch \<notin> chs \<Longrightarrow>
   pn \<in> pns1 \<Longrightarrow>
   sync_gassn chs pns1 pns2 (wait_out_cg I ch Q) (init_single pns2) s0 \<Longrightarrow>\<^sub>G
   wait_out_cg I ch (\<lambda>d v. sync_gassn chs pns1 pns2 (Q d v) (init_single pns2)) s0"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    subgoal for s11 s12 s21 s22 tr1 tr2
...
lemma sync_gassn_outv_emp:
  "ch \<notin> chs \<Longrightarrow>
   ...
   sync_gassn chs pns1 pns2 (wait_out_cgv I ch pns1 pn e Q) (init_single pns2) s0 \<Longrightarrow>\<^sub>G
   wait_out_cgv I ch (pns1 \<union> pns2) pn e (\<lambda>d. sync_gassn chs pns1 pns2 (Q d) (init_single pns2)) s0"
  unfolding wait_out_cgv_def
  apply (rule entails_g_trans)
...
lemma sync_gassn_out_emp_unpair:
  "ch \<in> chs \<Longrightarrow>
   sync_gassn chs pns1 pns2 (wait_out_cg I ch Q) (init_single pns2) s0 \<Longrightarrow>\<^sub>G P"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    subgoal for s11 s12 s21 s22 tr1 tr2
...
lemma sync_gassn_outv_emp_unpair:
  "ch \<in> chs \<Longrightarrow>
   sync_gassn chs pns1 pns2 (wait_out_cgv I ch pns1 pn e Q) (init_single pns2) s0 \<Longrightarrow>\<^sub>G P"
  unfolding wait_out_cgv_def
  apply (rule sync_gassn_out_emp_unpair) by auto

lemma sync_gassn_emp_in_unpair:
  "ch \<in> chs \<Longrightarrow>
   sync_gassn chs pns1 pns2 (init_single pns1) (wait_in_cg I ch Q) s0 \<Longrightarrow>\<^sub>G P"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    subgoal for s11 s12 s21 s22 tr1 tr2
...
lemma sync_gassn_in_emp:
  "ch1 \<notin> chs \<Longrightarrow>
   pns1 \<inter> pns2 = {} \<Longrightarrow>
   sync_gassn chs pns1 pns2 (wait_in_cg I1 ch1 P) (init_single pns2) s0 \<Longrightarrow>\<^sub>G
   wait_in_cg_alt (merge_inv I1 I2) ch1 pn (\<lambda>_ . 0)
    (\<lambda>d v. sync_gassn chs pns1 pns2 (P d v) (init_single pns2))
    (\<lambda>d. false_gassn) s0"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    subgoal for s11 s12 s21 s22 tr1 tr2
      ...
      subgoal for v tr1'
        apply (elim init_single.cases) apply auto
        apply (elim combine_blocks_emptyE3)
            apply auto
      ...
      subgoal for d v tr1' p
        apply (elim init_single.cases) apply auto
        apply (elim combine_blocks_emptyE2)
        done
      done
    done
  done
...
lemma sync_gassn_in_unpair_left:
  "ch1 \<notin> chs \<Longrightarrow>
   ...
   pns1 \<inter> pns2 = {} \<Longrightarrow>
   sync_gassn chs pns1 pns2 (wait_in_cg I1 ch1 P) (wait_out_cg I2 ch2 Q) s0 \<Longrightarrow>\<^sub>G
   wait_in_cg (merge_inv I1 I2) ch1
     (\<lambda>d v. sync_gassn chs pns1 pns2 (P d v) (wait_out_cg (delay_inv d I2) ch2 (\<lambda>d2. Q (d2 + d)))) s0"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    subgoal for s11 s12 s21 s22 tr1 tr2
      ...
      subgoal for d v tr1'
        ...
        subgoal for d2 tr2'
          ...
          subgoal
            ...
            apply (rule sync_gassn.intros) apply auto
             apply (rule wait_out_cg.intros) apply auto
             apply (rule delay_inv.intros) apply auto
            apply (rule merge_inv.intros) by auto
          subgoal apply auto
      ...
      subgoal for d v tr1' p
        ...
        subgoal for tr2' p'
          ...
          subgoal
            ...
            apply (rule wait_in_cg_alt.intros(3)) apply (auto simp add: merge_state_eval2)
             apply (rule sync_gassn.intros) apply auto
             apply (rule wait_in_cg.intros(2))
            by (auto intro: delay_inv.intros merge_inv.intros)
          done
        subgoal
...
lemma sync_gassn_out_unpair0:
  "ch1 \<notin> chs \<Longrightarrow>
   ...
   sync_gassn chs pns1 pns2 (wait_in_cg0 I1 ch1 pns1 P) (wait_out_cg I2 ch2 Q) s0 \<Longrightarrow>\<^sub>G
   wait_in_cg0 (merge_inv I1 I2) ch1 (pns1 \<union> pns2) (\<lambda>v. sync_gassn chs pns1 pns2 (P v) (wait_out_cg I2 ch2 Q)) s0"
...
lemma sync_gassn_out_unpair_wait0:
  "ch1 \<notin> chs \<Longrightarrow>
   ...
   sync_gassn chs pns1 pns2
     (wait_in_cg0 I1 ch1 pns1 P) (wait_cg I2 pn2 e Q) s0 \<Longrightarrow>\<^sub>G
   wait_in_cg_alt (merge_inv I1 I2) ch1 pn2 e
     (\<lambda>d v. (IFG (\<lambda>s. d = 0) THEN sync_gassn chs pns1 pns2 (P v) (wait_cg I2 pn2 e (\<lambda>d'. Q (d + d')))
...
lemma sync_gassn_emp_wait:
  assumes "pn\<in>pns2 \<and> pns1\<inter>pns2={}"
    and "e (the (s0 pn)) > 0"
  shows "sync_gassn chs pns1 pns2 (init_single pns1) (wait_cg I pn e Q) s0 \<Longrightarrow>\<^sub>G P"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    subgoal for s11 s12 s21 s22 tr1 tr2
      ...
      apply (elim wait_cg.cases) apply auto
      subgoal for tr' p
        by(elim combine_blocks_emptyE2')
      using assms
      by(auto simp add: merge_state_eval2)
    done
  done
      
lemma sync_gassn_wait_emp:
  assumes "pn\<in>pns1 "
    and "e (the (s0 pn)) > 0"
  shows "sync_gassn chs pns1 pns2 (wait_cg I2 pn e Q) (init_single pns2) s0 \<Longrightarrow>\<^sub>G P"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    subgoal for s11 s12 s21 s22 tr1 tr2
      ...
      subgoal for tr' p
        by(elim combine_blocks_emptyE2)
      using assms
      by(auto simp add: merge_state_eval1)
    done
  done
...
lemma sync_gassn_in_alt_out:
  "ch \<in> chs \<Longrightarrow>
   ...
   d > 0 \<Longrightarrow>
   sync_gassn chs pns1 pns2
      (wait_in_cg_alt I1 ch pn1 (\<lambda>_. d)
        (\<lambda>d v. IFG (\<lambda>s. d = 0) THEN P v ELSE true_gassn pns1 FI)
        (\<lambda>v. true_gassn pns1))
      (wait_out_cg I2 ch Q) s0 \<Longrightarrow>\<^sub>G
  ...
  "ch \<in> chs \<Longrightarrow>
   ...
   sync_gassn chs pns1 pns2
      (wait_in_cg_alt I1 ch pn1 (\<lambda>_. d)
        (\<lambda>d v. IFG (\<lambda>s. d = 0) THEN P v ELSE true_gassn pns1 FI)
        (\<lambda>v. true_gassn pns1))
      (wait_out_cgv I2 ch pns2 pn3 e Q) s0 \<Longrightarrow>\<^sub>G
...
lemma sync_gassn_out_wait_pair:
  "ch \<in> chs \<Longrightarrow>
   pn \<in> pns2 \<Longrightarrow>
   pns1 \<inter> pns2 = {} \<Longrightarrow>
   sync_gassn chs pns1 pns2
     (wait_out_cg I1 ch P) (wait_cg I2 pn e Q) s0 \<Longrightarrow>\<^sub>G
   wait_cg (merge_inv I1 I2) pn e
     (\<lambda>d. sync_gassn chs pns1 pns2 (wait_out_cg (delay_inv d I1) ch (\<lambda>d' v. P (d + d') v)) (Q d)) s0"
  unfolding entails_g_def apply auto
  subgoal for s tr
    apply (elim sync_gassn.cases) apply auto
    subgoal for s11 s12 s21 s22 tr1 tr2
      ...
      subgoal for v tr1'
        ...
        subgoal
          ...
          subgoal apply (subst merge_state_eval2) by auto
          apply (rule sync_gassn.intros) apply auto
          apply (rule wait_out_cg.intros(1)) apply auto
          done
        done
      subgoal for d v tr1' p
        apply (elim wait_cg.cases) apply auto
        ...
        subgoal
          ...
          subgoal apply (subst merge_state_eval2) by auto
          apply (rule sync_gassn.intros) apply auto
          apply (rule wait_out_cg.intros(2)) apply auto
          done
        done
      done
    done
