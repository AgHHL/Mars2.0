...
lemma interrupt_solInf_emp:
  "interrupt_solInf_cg I [] = false_gassn"
  unfolding false_gassn_def
  apply rule+
   apply auto
  apply(elim interrupt_solInf_cg.cases)
     apply auto
  done
...
fun spec_wait_of :: "real \<Rightarrow>  'a comm_specg2 \<Rightarrow> 'a comm_specg2" where
  "spec_wait_of d (InSpecg2 ch P) = InSpecg2 ch (\<lambda>d' v. P (d + d') v)"
| "spec_wait_of d (OutSpecg2 ch P) = OutSpecg2 ch (\<lambda>d' v. P (d + d') v)"
...
lemma rdy_of_spec_wait_of [simp]:
  "rdy_of_comm_specg2 (map (spec_wait_of d) specs) = rdy_of_comm_specg2 specs"
...
lemma merge_rdy_comm_specg2_empty:
  ...
  using assms
proof(induction "length specs" arbitrary: specs)
  case 0
  then show ?case 
    apply auto
  ...
  then show ?case 
    ...
    subgoal for a l
      ...
      subgoal for ch c
        ...
        using Suc(1)[of l] 
        apply auto
        apply(subgoal_tac"(\<And>i. i < length l \<Longrightarrow> ch_of_specg2 (l ! i) \<in> chs)")
         apply auto
         apply(subgoal_tac"ch\<in>chs")
        subgoal 
          unfolding rdy_of_comm_specg2_def
          apply(cases "rdy_info_of_list (case_comm_specg2 (\<lambda>ch P. ({}, {ch})) (\<lambda>ch P. ({ch}, {}))) l")
          by auto
        subgoal
          by (metis Nat.lessE append_eq_Cons_conv ch_of_specg2.simps(1) length_Suc_conv list.distinct(1) not_less_eq nth_append_length)
        subgoal for i
          apply(subgoal_tac"(l ! i) = (InSpecg2 ch c # l) ! (i+1)")
          subgoal 
      ...
      subgoal for ch c
        ...
        subgoal 
          ...
          by auto
        subgoal
          by (metis ch_of_specg2.simps(2) nth_Cons_0 zero_less_Suc)
        subgoal for i
          apply(subgoal_tac"(l ! i) = (InSpecg2 ch c # l) ! (i+1)")
          subgoal 
            by (metis Suc_mono nth_Cons_Suc)
...
lemma not_compat_rdy_comm_specg2:
  ...
  apply(subgoal_tac "ch\<in>snd (rdy_of_comm_specg2 specs)")
  subgoal
    apply(cases "(rdy_of_comm_specg2 specs)")
    by auto
  using assms
...
lemma sync_gassn_interrupt_solInf_wait:
  ...
  "sync_gassn chs pns1 pns2 (interrupt_solInf_cg I1 specs) (wait_cg I2 pn2 e Q) s0 \<Longrightarrow>\<^sub>G
   wait_cg (merge_inv I1 I2) pn2 e
    (\<lambda>d. sync_gassn chs pns1 pns2 (interrupt_solInf_cg (delay_inv d I1) (map (spec_wait_of d) specs)) (Q d)) s0"
