...
lemma ex5':
  ...
  subgoal
    ...
    unfolding ex5_one_step_def apply (auto simp add: A_def B_def restrict_state_def)
    unfolding epartg_def by simp
...
lemma ex6:
  "spec_of_global
    ...
    (wait_in_cg_alt (id_inv {B, A}) ch2 B (\<lambda>_. 1)
      (\<lambda>d v. IFG (\<lambda>s. d = 0) THEN
  ...
  (* Stage 3: combine the two assertions *)
    apply (rule entails_g_trans)
     apply (rule sync_gassn_out_unpair_wait0)
            apply (auto simp add: ch1_def ch2_def A_def B_def)[7]
    apply simp
...
lemma ex6_full:
  ...
  (* Stage 3: combine the two assertions *)
    apply (rule entails_g_trans)
     apply (rule sync_gassn_in_alt_outv) apply (auto simp add: A_def B_def C_def)[5]
    apply (rule entails_g_trans)
