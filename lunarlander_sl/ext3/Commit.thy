...
definition Validc :: "cname set \<Rightarrow> assn \<Rightarrow> ctr \<Rightarrow> proc \<Rightarrow> assn \<Rightarrow> ctr \<Rightarrow> bool" 
("\<Turnstile>c(_) ({(1_),(2_)}/ (_)/ {(1_),(2_)})" 70) where
...
" ch\<in>chs \<Longrightarrow>
  \<Turnstile>c(chs) {\<lambda> s t. s = ss \<and> P s t,cI ch (e ss)} Cm (ch[!]e) 
    {(\<lambda> s t. s = ss \<and> (P s @\<^sub>t Out0\<^sub>t ch (e ss)) t),(cO ch (e ss))}"
...
" ch\<in>chs \<Longrightarrow> d>0 \<Longrightarrow> 
  \<Turnstile>c(chs) {\<lambda> s t. s= ss \<and> P s t,cW d ({},{}) ({},{ch}) [@] cI ch (e ss)} Cm (ch[!]e) 
   {(\<lambda> s t. s=ss \<and> (P s @\<^sub>t (Wait\<^sub>t d (\<lambda>_ . State ss) ({ch},{})) @\<^sub>t Out0\<^sub>t ch (e ss)) t),
                     (cW d ({ch},UNIV-{ch}) ({},UNIV) [@]cO ch (e ss))}"
  unfolding Validc_def apply (auto simp add:join_assn_def)
  subgoal 
...
lemma Validc_in1:
" ch\<in>chs \<Longrightarrow>
  \<Turnstile>c(chs) {\<lambda> s t. s = ss \<and> P s t,cO ch v} Cm (ch[?]x) 
    {(\<lambda> s t. s = ss(x := v) \<and> (P ss @\<^sub>t In0\<^sub>t ch v) t),(cI ch v)}"
  unfolding Validc_def apply (auto simp add:join_assn_def)
  subgoal 
    apply(auto elim!: receiveE commitOE)
    using combine_blocks_pairE apply blast
    using combine_blocks_pairE2' by blast
  subgoal for tr1 s2 tr2 tr2' tr3 tr3' x
    apply(rule exI[where x= tr1])
    apply(auto elim!: receiveE commitOE)
...
lemma Validc_wait1:
" \<Turnstile>c(chs) {\<lambda> s t. s = ss \<and> P s t,if e ss \<le>0 then cE else cW (e ss) ({},{}) ({},{})} 
          Wait e 
        {(\<lambda> s t . s = ss \<and> (P s @\<^sub>t Wait\<^sub>t (e ss) (\<lambda>_ . State s) ({},{})) t),
                (if e ss \<le> 0 then cE else cW (e ss) ({},UNIV) ({},UNIV))}"
  unfolding Validc_def apply (auto simp add:join_assn_def)
  subgoal 
    by(auto elim!: waitE)
  subgoal for tr1 s2 tr2 tr2' tr3 tr3' x
...
lemma Validc_repn1:
  assumes"\<And> i. \<Turnstile>c(chs) {\<lambda> s t. s = ss i \<and> P s t,a (i+1)} 
                                  S 
                     {\<lambda> s t. s = ss (i+1) \<and> P s t,g (i+1)} "
  shows " \<Turnstile>c(chs) {\<lambda> s t. s = ss 0 \<and> P s t,crepn N a} 
                           RepN N S 
                {\<lambda> s t. s = ss N \<and> P s t,crepn N g}"
