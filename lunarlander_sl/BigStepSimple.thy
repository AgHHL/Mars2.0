...
theorem Valid_post_and:
  "\<Turnstile> {P} c {Q} \<Longrightarrow> \<Turnstile> {P} c {R} \<Longrightarrow> \<Turnstile> {P} c {\<lambda> s tr. Q s tr \<and> R s tr}"
  unfolding Valid_def by auto

theorem Valid_pre_or:
  "\<Turnstile> {P} c {R} \<Longrightarrow> \<Turnstile> {Q} c {R} \<Longrightarrow> \<Turnstile> {\<lambda> s tr. P s tr \<or> Q s tr} c {R}"
  unfolding Valid_def by auto
