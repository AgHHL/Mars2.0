...
def runFile(self: unittest.TestCase, file: str, *,
            print_vcs: bool = False,
            expected_vcs: dict[tuple, str] = None):
    """Run test on the given file.
    
    file : str
        name of the file in examples folder.
    print_vcs : bool
        whether to print list of vcs (including other information).
    expected_vcs : dict[tuple, str]
        if present, check the list of VCs are agrees with provided.

    """
    # Read the file
    path = os.path.join(os.path.dirname(__file__), "../examples", file)
    ...
    def is_trivial(vc: expr.Expr):
        """Trivial conditions are of the form A -> A, these are skipped
        when comparing expected vcs.
        
        """
        return isinstance(vc, expr.LogicExpr) and vc.op == "->" and \
            vc.exprs[0] == vc.exprs[1]
    ...
    if expected_vcs:
        for pos, vcs in expected_vcs.items():
            ...
            actual_vcs = [vc.expr for vc in verifier.infos[pos].vcs
                          if not is_trivial(vc.expr)]
            self.assertEqual(
                set(vcs), set(actual_vcs),  "\nExpect: {}\nActual: {}".format(
                    [str(vc) for vc in vcs],
                    [str(vc) for vc in actual_vcs]))
...
class HHLPyTest(unittest.TestCase):
    ...
    def testReplaceFunction1(self):
        ...
        self.assertEqual(str(hcsp.replace_function(res, funcs)), 'y + 1')
    ...
    def testReplaceFunction2(self):
        ...
        self.assertEqual(str(hcsp.replace_function(res, funcs)), '2 * a == b')
    ...
    def testReplaceFunction3(self):
        ...
        self.assertEqual(str(hcsp.replace_function(res, funcs)), '2 * bar(x)')

    def testVerify1(self):
        runFile(self, file="test1.hhl")

    @unittest.skipUnless(found_wolfram, "wolfram kernel not installed")
...
class NonlinearHHLPyTest(unittest.TestCase):
    ...
    def testpi(self):
        runFile(self, file="testpi.hhl")

    def examplepi(self):
        runFile(self, file="examplepi.hhl")
    ...
    def examplebc(self):
        runFile(self, file="examplebc.hhl")
...
class AdvancedHHLPyTest(unittest.TestCase):
    ...
    def testSkydiver(self):
        runFile(self, file="skydiver.hhl")
