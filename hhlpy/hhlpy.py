...
from sympy import sympify, degree, symbols, fraction, simplify
...
from ss2hcsp.hcsp import label
    ...
    def rec(e):
        if isinstance(e, expr.LogicExpr):
            ...
            if len(e.exprs) == 0:
                ...
                return expr.OpExpr("*", expr.FunExpr("cos", [e.exprs[0]]), rec(e.exprs[0]))
            elif e.fun_name == "cos":
                return expr.OpExpr("-", expr.OpExpr("*", expr.FunExpr("sin", [e.exprs[0]]), rec(e.exprs[0])))
            else:
                raise NotImplementedError
...
class Condition:
    """Stores a condition (including pre-condition, post-condition, verification
    condition) and the parts of the program that it originates from.
    
    """
    def __init__(self, expr, path=[], origins=[], blabel=None, blabel_depth=0,
                 bottom_loc=None, categ=None, isVC=False):
...
class OriginLoc:
    """Location of an assertion (pre-condition, post-condition, invariant) or
    expression that a Condition originates from.
    
    """
    def __init__(self, index=None, isPre=False, isPost=False, isInv=False, isGhost=False,
                 isConstraint=False, hp_pos=None):
...
class CmdVerifier:
    ...
    def __init__(self, *, pre: Iterable[OrdinaryAssertion], hp: hcsp.HCSP,
                 post: Iterable[OrdinaryAssertion],
                 constants: hcsp.Constants,
                 functions: Optional[hcsp.Function] = None):
        ...
        self.infos: dict[tuple, CmdInfo] = dict()

        # The conjunction of post expression
        pre_conj: expr.Expr = expr.list_conj(*[subpre.expr for subpre in pre])

        # The conjunction of post expression
        post_conj: expr.Expr = expr.list_conj(*[subpost.expr for subpost in post])
        ...
        for subpre in self.infos[root_pos].pre:
            ...
            subpre_expr = hcsp.subst_all_funcs(subpre_expr, self.functions)
    ...
    def compute_vc_ode_bc(self, sub_pos, constraint, inv, origins):
        ...
        while isinstance(barrier_inv, expr.LogicExpr) and barrier_inv.op == "&&":
            conexpr = barrier_inv.exprs[0]
            assert isinstance(conexpr, expr.RelExpr) and conexpr.op in {'<=', '>=', '>', '<'}
            barrier_invs.append(conexpr)
            barrier_inv = barrier_inv.exprs[1]
        ...
        for barrier_inv in barrier_invs:
            # Translate '<=' into '>='
            if barrier_inv.op == '<=':
            ...
            if barrier_inv.expr2 != expr.AConst(0):
            ...
            es.append(e)
            e_lies.append(e_lie)
            nbarrier_invs.append(barrier_inv)
        ...
        for i in range(0,len(es)):
            ...
            for j in range(0,len(es)):
                if j == i:
                    vc = expr.LogicExpr('&&', vc, expr.RelExpr('==', es[i], expr.AConst(0)))
                else:
                    vc = expr.LogicExpr('&&', vc, nbarrier_invs[j])
            gra = []
            for key in self.infos[sub_pos].eqs_dict:
                dic_g ={}
                dic_g[key] = expr.AConst(1)
                g = compute_diff(es[i], eqs_dict=dic_g, functions=self.functions)
                gra.append(g)
            for j in range(0,len(gra)):
                if j == 0:
                    vg = expr.RelExpr('!=', gra[j], expr.AConst(0))
                else:
                    vg = expr.LogicExpr('||', vg, expr.RelExpr('!=', gra[j], expr.AConst(0)))

            vc = expr.imp(vc, expr.LogicExpr('&&', vg, expr.RelExpr('>=', e_lies[i], expr.AConst(0))))
            vcl.append(vc)
    ...
    def get_all_vcs(self) -> dict[tuple, list[Condition]]:
        all_vcs: dict[tuple, list[Condition]] = dict()
        for pos, info in self.infos.items():
            if info.vcs:
