...
if path:
    ...
    print("Please install Wolfram Kernel and specify its location in the environment variable \"WolframKernel\".")
    ...
    def rec(e):
        if isinstance(e, expr.AVar):
            ...
            if e.fun_name in functions:
                return rec(hcsp.replace_function(e, functions))
            if e.fun_name == "cos":
                return wl.Cos(rec(e.exprs[0]))
            if e.fun_name == "sin":
                return wl.Sin(rec(e.exprs[0]))
            ...
            return wl.If(rec(e.cond), rec(e.expr1), rec(e.expr2))
        else:
            raise NotImplementedError
...
def toHcsp(e):
    ...
    if isinstance(e, WLFunction):
        ...
        # Translate into LogicExpr in hcsp.
            ...
            assert len(e.args) == 2
            return expr.LogicExpr('<->', toHcsp(e.args[0]), toHcsp(e.args[1]))
        elif e.head == WLSymbol("Inequality"):
            assert len(e.args) == 5
            return expr.LogicExpr('&&', toHcsp(WLFunction(e.args[1], e.args[0], e.args[2])), 
                                  toHcsp(WLFunction(e.args[3], e.args[2], e.args[4])))
...
def solveODE(hp: hcsp.HCSP, names: set[str], time_var: str) -> dict[str, Expr]:
    """Return the mapping from variable to its solution.
...
def Ode2Wlexpr(hp: hcsp.HCSP, names: set[str], time_var: str):
    """
...
def wl_prove(e: Expr, functions: dict[str, Function] = None) -> bool:
    """Attempt to prove the given condition by reducing it to true using
    WolframEngine.
    
    Return True if the condition can be reduced to true. Return False if
    the condition reduces to false or cannot be reduced to a boolean constant.
    ...
    if not found_wolfram:
        raise AssertionError("wolfram kernel is not installed")
    ...
    vars = hcsp.subst_all_funcs(e, funcs=functions).get_vars()
...
def wl_simplify(e: Expr, functions: dict[str, Function] = None):
...
def wl_polynomial_div(p, q, constants, functions=dict()):
    ...
    vars = hcsp.subst_all_funcs(q, functions).get_vars().difference(constants)
...
def wl_is_polynomial(e, constants=set(), functions=dict()):
    ...
    vars = hcsp.subst_all_funcs(e, functions).get_vars().difference(constants)
...
def wl_reduce(e: Expr, vars: list[str], functions: dict[str, Function] = None) -> expr.Expr:
    """Attempt to reduce the given expression using WolframEngine."""
    ...
    result = session.evaluate(wl.Reduce(wl_expr, wl_vars,wl.Reals))    
    remain = toHcsp(result)
    return remain

def wl_reduce_true(e: Expr, functions: dict[str, Function] = None):
    """Attempt to prove the given condition is true by reducing it
    using WolframEngine. This is a simplified version of wl_prove.
...
def wl_reduce_false(e: Expr, functions: dict[str, Function] = None):
    """Attempt to prove the given condition is false by reducing it
    using WolframEngine. This is a simplified version of wl_proven.
...
def wl_proven(e: Expr, functions: dict[str, Function] = None) -> bool:
    """Attempt to disprove the given condition by reducing it to false
    using WolframEngine.
    
    Return True if the condition can be reduced to false, Return False
    if the condition reduces to true or cannot be reduced to a boolean
    constant.
    ...
    if wl_vars:
        ...
        # because that Reduce cannot reduce the VCs in basic 46, 47 into true，
        # even though the VCs should be valid.
        result = session.evaluate(wl.FindInstance(wl.Simplify(wl_expr), wl_vars, wl.Reals))
        # result is an empty tuple, i.e. no instance found for the not expression.
        if not result:
            return True
        else:
