...
from ss2hcsp.sl.SignalRouting import merge
...
class Triggered_Subsystem(Subsystem):
    ...
    def __repr__(self):
        return "Triggered_Subsystem(%s, %s, %s, %s, in = %s, out = %s, st = %s)" % \
            (self.name, self.num_src, self.num_dest, self.trigger_type,
             str(self.dest_lines), str(self.src_lines), self.st)
    ...
    def get_init_hps(self):
        ...
        # 触发子系统未展开时的前置状态赋值
        for num in range(self.num_src):
            out_var = self.src_lines[0][0].name
            init_hps.append(hp.Assign(var_name=out_var + '_' + str(num),expr=AConst(0)))
    ...
    def get_procedures(self, mergeblock = None):
        ...
        # input and output handle
        block_dict_outport = {name: block for name, block in self.diagram.blocks_dict.items()
                           if block.type in ["out_port"] }
        if len(block_dict_outport) != 0:
            output_hps.append(hp.Assign(self.src_lines[0][0].name, hp.AConst(1)))
            for block in block_dict_outport.values():
                output_hps.append(hp.Assign(self.src_lines[0][0].name + '_' + block.name.split('_')[1],AVar(block.dest_lines[0].name)))
        ...
        if len(result_hps) > 1:
            result_hp = hp.Sequence(*result_hps)
        elif len(result_hps) == 1:
            result_hp = result_hps[0]
        else:
            result_hp = hp.Skip()
