...
from ss2hcsp.sl.SignalRouting.merge import Merge
from ss2hcsp.sl.SignalRouting.terminator import Terminator
...
from ss2hcsp.sl.SignalAttributes.ratetransition import RateTransition
...
class SL_Diagram:
    ...
    def __init__(self, location=""):
        ...
        # Trigger the internal list of the subsystem
        self.trigger_subsystem_block_dict = []
            ...
            for child in children[0].childNodes:
                if child.nodeName == "state":
                    ...
                    if state_type == "FUNC_STATE":
                        ...
                        # add support for simulink func here
    ...
    def parse_xml(self, block_parameters=(),mask_parameters=()):
        ...
        if not block_parameters:
            ...
            for child in default_para_blocks[0].childNodes:
                if child.nodeName == "Block":
                    ...
                    block_parameters[child_type]["InitialOutput"] = get_attribute_value(child,"InitialOutput")
                    block_parameters[child_type]["InitialCondition"] = get_attribute_value(child,"InitialCondition")
        ...
        for block in blocks:
            ...
            if block_type == "Mux":
                ...
                if value in mask_parameters.keys():
                    value = mask_parameters[value]
                else:
                    value = parse_value(value, default=1) #加参数
                ...
                if block_type and block_type == 'Compare To Constant':
                    relop = get_attribute_value(block, "relop")
                    value = get_attribute_value(block,'const')
                    if relop:
                        if relop == "~=":
                            relop = "!="
                        self.add_block(Reference(name=block_name,relop=relop,value=value,sourcetype=block_type,st=sample_time))
                    continue
                ...
                if sample_time == -1:
                    sample_time = int(block_parameters['UnitDelay']['SampleTime'])
                ...
                for child in system.childNodes:
                    if child.nodeName == "Line":
                        if get_attribute_value(block=child, attribute="DstBlock", name=block_name):
                            # line message in Line
                            name = get_attribute_value(block=child, attribute="Name")
                            if not name:
                                # line message in Branch
                                for children in child.childNodes:
                                    if children.nodeName == "Branch":
                                        name = get_attribute_value(block=children,attribute="Name")
                            if not name:
                                # else no scope name in model
                                name = block_name
                ...
                if triggers:  # it is a triggered subsystem
                    ...
                    if not get_attribute_value(enables[0],"StatesWhenEnabling"):
                        assert get_attribute_value(enables[0], "StatesWhenEnabling") is None
                    if not get_attribute_value(enables[0],"PropagateVarSize"):
                ...
                if triggers:
                    self.trigger_subsystem_block_dict.append(subsystem.diagram.blocks_dict)
                ...
                init_value = parse_value(get_attribute_value(block,"InitialOutput"))
                ports = get_attribute_value(block,"Ports")
                num_dest, num_src = [int(port.strip("[ ]")) for port in ports.split(",")]
                self.add_block(Merge(block_name, num_src, num_dest, init_value))
            elif block_type == "RateTransition":
                st = parse_value(get_attribute_value(block,"OutPortSampleTime"))
                if not get_attribute_value(block,"InitialCondition"):
                    init_value = int(block_parameters[block_type]["InitialCondition"])
                else:
                    init_value = parse_value(get_attribute_value(block,"InitialCondition"))
                self.add_block(RateTransition(block_name,init_value,st))
            elif block_type == "Terminator":
                self.add_block(Terminator(block_name,1))
            else:
    ...
    def trigger_subsystem_add_line_name(self):
        if len(self.trigger_subsystem_block_dict) != 0:
            for block in self.trigger_subsystem_block_dict[0].values():
                if block.type == 'out_port' and str(block.dest_lines[0].name) != '?':
                    return None

            flag = 'x0'
            for name,value in self.blocks_dict.items():
                for i in value.dest_lines:
                    if i.name != '??' and parse_value(i.name.split('x')[1]) > parse_value(flag.split('x')[1]):
                        flag = i.name

                for i in value.src_lines:
                    if i[0].name != '??' and parse_value(i[0].name.split('x')[1]) > parse_value(flag.split('x')[1]):
                        flag = i[0].name

            num_lines = parse_value(flag.split('x')[1]) + 1

            for blocks in self.trigger_subsystem_block_dict:
                for name,block in blocks.items():
                    ...
                    for line in block.dest_lines:
                        if line:
                            ...
                            line_group = blocks[src].src_lines[src_port]
        ...
        for block in self.blocks_dict.values():
            ...
            if block.type in ["subsystem", "enabled_subsystem"]:
                ...
                for port_id in range(block.num_dest):
                    ...
                    # reference subsystem_flag assignment
                    if (src_block.type == 'reference' and self.blocks_dict[src_block.src_lines[0][0].dest].type
                            in ["enabled_subsystem"]):
                        src_block.subsystem_flag = src_block.src_lines[0][0].dest
