...
def translate_discrete(diagram: List[SL_Block], chart_parameters):
    ...
    for block in sorted_blocks:
        if block.st == sample_time:
            ...
            #reference'name ?? special handle
            if block.type == 'reference' and block.subsystem_flag:
                for block1 in sorted_blocks:
                    if block.subsystem_flag in block1.name:
                        cur_sig = block1.dest_lines[0].name
                        break
                relop = block.get_expr()
                pre_sig = block.dest_lines[0].name
                trig_cond = RelExpr(relop, AVar(pre_sig), AConst(int(block.value)))
                output_hps.append(hp.ITE([(trig_cond, hp.Assign(cur_sig, AConst(0)))]))
            else:
        ...
        ##############################
            ...
            update_hps.append(hp.Assign(block.src_lines[0][0].name,AConst(0)))
