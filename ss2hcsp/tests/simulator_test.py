...
def run_test(self: unittest.TestCase, infos, num_events, trace, *, io_filter=None,
    ...
    if isinstance(infos, list):
        for i in range(len(infos)):
            if isinstance(infos[i], str):
                # Single HCSP program
                sim_infos.append(simulator.SimInfo('P' + str(i), parser.parse_hp(infos[i])))
            else:
...
class SimulatorTest(unittest.TestCase):
    ...
    def testExecStep(self):
        ...
        for cmd, pos, state, pos2, state2 in test_data:
            info = simulator.SimInfo('P0', parser.parse_hp(cmd), pos=pos, state=state)
            info.exec_step()
    ...
    def testODEComm(self):
        ...
        for cmd, state, delay, state2 in test_data:
            info = simulator.SimInfo('P0', parser.parse_hp(cmd), state=state)
            info.exec_step()  # obtain delay value
            info.exec_delay(delay)
    ...
    def run_test_trace(self, infos, *, num_steps, num_show, ids=None,
        ...
        for i in range(len(infos)):
            infos[i] = simulator.SimInfo('P' + str(i), parser.parse_hp(infos[i]))
