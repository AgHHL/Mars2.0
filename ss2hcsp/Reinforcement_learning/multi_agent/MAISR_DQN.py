...
from ss2hcsp.Reinforcement_learning.multi_agent.MADQN import MultiAgentDeepQNetwork, MultiAgentDeepQNetworkExternal
import copy
...
class ISR_Test(unittest.TestCase):
    def run_processes(self, infos: list[HCSPInfo], num_io_events, *, outputs):
        ...
        network = MultiAgentDeepQNetwork(
            4, (10, 9),
            learning_rate=0.01,
            reward_decay=0.9,
            e_greedy=0.9,
        ...
        network_external = MultiAgentDeepQNetworkExternal(network)
        ...
        for _ in range(10):
            ...
            last_state_1 = p1_value[-1]['state']['success_1']
            last_state_2 = p1_value[-1]['state']['success_2']
            print(last_state_1, last_state_2)
            print(last_num_step)
    ...
    def test_ISRdqn(self):
        with open("ss2hcsp/Reinforcement_learning/multi_agent/ISR_MA.txt", "r") as f:
            text = f.read()
        infos = parse_module_file(text)
        res, events = self.run_processes(infos, 10000000, outputs=['map','num_step','success_1','success_2'])
