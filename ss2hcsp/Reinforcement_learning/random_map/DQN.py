...
import ast
from ss2hcsp.Reinforcement_learning.random_map.generate_map import MapGenerator
...
class DeepQNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        self.start_pos = MapGenerator().generate_start_pos(x=self.map_x, y=self.map_y)
        self.end_pos = MapGenerator().generate_end_pos(x=self.map_x, y=self.map_y, start_pos=self.start_pos)
        self.mmap = MapGenerator().generate_map(x=self.map_x, y=self.map_y, start_pos=self.start_pos, end_pos=self.end_pos, types=1)
    ...
    def get_start_pos(self):
        #print("start_pos is :", list(self.start_pos))
        return list(self.start_pos)

    def get_end_pos(self):
        #print("end_pos is :", list(self.end_pos))
...
class DeepQNetworkExternal(SimulatorExternal):
    def __init__(self, network: DeepQNetwork):
        ...
        self.operations = ("choose_action", "store_transition", "learn", "get_fullmap", "init_agent", "get_global_step", "get_start_pos", 
                           "get_end_pos", "get_map")
    ...
    def exec(self, operation_name: str, args: list):
        ...
        elif operation_name == "get_start_pos":
            assert len(args) == 0
            return self.network.get_start_pos()
        elif operation_name == "get_end_pos":
            assert len(args) == 0
            return self.network.get_end_pos()
        elif operation_name == "get_map":
            assert len(args) == 0
            return self.network.get_map()
        else:
