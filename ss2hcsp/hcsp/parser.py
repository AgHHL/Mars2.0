...
grammar = r"""
    ...
    ?atom_cmd: "@" CNAME ";" -> var_cmd
        ...
        | "$$" CNAME "(" ")" ";" -> external_cmd
        | "$$" CNAME "(" expr ("," expr)* ")" ";" -> external_cmd_with_args
        | lname ":=" "$$" CNAME "(" ")" ";" -> assign_external_cmd
        | lname ":=" "$$" CNAME "(" expr ("," expr)* ")" ";" -> assign_external_cmd_with_args
        ...
        | "{" ode_seq "&" expr "+>" cmd "}" "|>" "[]" "(" interrupt ")" maybe_ode_invariant -> ode_commn
    ...
    ?atom_label: INT
        ...
        | "select_action"             -> atom_label_select_action
        | "learn"             -> atom_label_learn
        | "store_transition"             -> atom_label_store_transition
    ...
    ?cmd: select_cmd
    ?pn_cmd: CNAME ":" cmd
    ?pparallel_cmd: "(" pn_cmd "|{}|" pn_cmd ")"
      |  "(" pn_cmd "|{}|" pparallel_cmd ")"
      |  "(" pparallel_cmd "|{}|" pn_cmd ")"
      |  "(" pparallel_cmd "|{}|" pparallel_cmd ")"
      |  "(" pn_cmd "|{" CNAME ("," CNAME)* "}|" pn_cmd ")"
      |  "(" pn_cmd "|{" CNAME ("," CNAME)* "}|" pparallel_cmd ")"
      |  "(" pparallel_cmd "|{" CNAME ("," CNAME)* "}|" pn_cmd ")"
      |  "(" pparallel_cmd "|{" CNAME ("," CNAME)* "}|" pparallel_cmd ")"

    ?function_decl: "function" CNAME "(" CNAME ("," CNAME)* ")" "=" expr ";"
    ?constants_decl: "constants" (expr ";")* "end"  -> constants_decl
    ?loopinv_decl: "loopinv" CNAME ":" expr ("," CNAME ":" expr)* "end"  -> loopinv_decl
    ?odeass_decl: "odeass" cmd

    ?hoare_pre : "pre" (ord_assertion)* ";" -> hoare_pre
    ?hoare_post : "post" (ord_assertion)* ";" -> hoare_post
    ?hoare_postr : "postr" (ord_assertion)* ";" -> hoare_postr
    ?hoare_triple: constants_decl? (function_decl)* hoare_pre cmd hoare_post  -> hoare_triple_seq
      | (function_decl)* loopinv_decl? (odeass_decl)* hoare_pre cmd hoare_post hoare_postr -> hoare_triple_par
      | (function_decl)* loopinv_decl? (odeass_decl)* hoare_pre pparallel_cmd hoare_post hoare_postr -> hoare_triple_par
...
class HPTransformer(Transformer):
    ...
    def external_cmd(self, meta, name: Token) -> hcsp.External:
        return hcsp.External(None, str(name), tuple(), meta=meta)
    
    def external_cmd_with_args(self, meta, name: Token, *args: Expr) -> hcsp.External:
        return hcsp.External(None, str(name), args, meta=meta)

    def assign_external_cmd(self, meta, var, name: Token) -> hcsp.External:
        return hcsp.External(var, str(name), tuple(), meta=meta)
    
    def assign_external_cmd_with_args(self, meta, var, name: Token, *args: Expr) -> hcsp.External:
        return hcsp.External(var, str(name), args, meta=meta)
    ...
    def unsol_intro(self, meta, *args):
        res = []
    ...
    def atom_label_select_action(self, meta): return "select_action"
    def atom_label_learn(self, meta): return "learn"
    def atom_label_store_transition(self, meta):return "store_transition"
    ...
    def ode(self, meta, eqs, constraint, asts):
        ...
        return hcsp.ODE(eqs, constraint, meta=meta, unsols=unsols, ghosts=tuple(ghosts), inv=tuple(inv))
    ...
    def ode_comm(self, meta, eqs, constraint, io_comms, asts):
        inv = []
        unsols = []
        for ast in asts:
            if isinstance(ast, assertion.CutInvariant):
                inv.append(ast)
            elif isinstance(ast, list):
                unsols = unsols + ast
            else:
                raise NotImplementedError
        return hcsp.ODE_Comm(eqs, constraint, io_comms, meta=meta, unsols=unsols, inv=tuple(inv))
    
    def ode_commn(self, meta, eqs, constraint, tail, io_comms, asts):
        inv = []
        unsols = []
        for ast in asts:
            if isinstance(ast, assertion.CutInvariant):
                ...
                raise NotImplementedError
        return hcsp.ODE_Commn(eqs, constraint, tail, io_comms, meta=meta, unsols=unsols, inv=tuple(inv))
    ...
    def pn_cmd(self, meta, pn, cmd):
        return hcsp.Pnseqhp(pn, cmd)
    
    def pparallel_cmd(self, meta, *args):
        chs = []
        for arg in args[1:-1]:
            chs.append(arg)
        return hcsp.PParallel(args[0], args[-1], chs)
    ...
    def loopinv_decl(self, meta, *args):
        dict = {}
        for i in range(0,len(args),2):
            dict[args[i]] = args[i+1]
        return dict
    
    def odeass_decl(self, meta, cmd):
        return cmd
    ...
    def hoare_postr(self, meta, *args):
        return list(args)

    def hoare_triple_seq(self, meta, *args) -> hcsp.HoareTriple:
    ...
    def hoare_triple_par(self, meta, *args) -> hcsp.HoareTriple:
        # The last three arguments are pre, hp, and post.
        pre = args[-4]
        hp = args[-3]
        post = args[-2]
        ...
        for item in args[:-4]:
            if isinstance(item, hcsp.Function):
                functions[item.name] = item
            elif isinstance(item, hcsp.HCSP):
                odeass.append(item)
            elif isinstance(item, dict):
                loopinv = item
            else:
                raise NotImplementedError
        return hcsp.HoareTriplePar(pre, hp, post, postr, functions=functions, loopinv=loopinv, odeass=odeass, meta=meta)
    ...
    def hcsp_import(self, meta, filename) -> module.HCSPDeclarations:
...
php_parser = Lark(grammar, start="pparallel_cmd", parser="lalr", transformer=hp_transformer)
...
def parse_php(text: str) -> HCSP:
    return php_parser.parse(text)
...
php_tree_parser = Lark(grammar, start="pparallel_cmd", parser="lalr", propagate_positions=True)
...
def parse_php_with_meta(text) -> HCSP:
    return hp_transformer.transform(php_tree_parser.parse(text))
...
def parse_file(text: str) -> list[hcsp.HCSPInfo]:
