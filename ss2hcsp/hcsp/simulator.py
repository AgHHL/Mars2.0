...
class SimulatorExternal:
    """Represents an external actor to simulator.
    
    Each external actor registers a list of operations. Each operation
    takes a list of arguments and may return an expression.

    Attributes
    ----------
    name : str
        name of the external, for display only.
    operations : tuple[str]
        list of operations supported by the external.
    ...
    def __str__(self):
        return f"Simulator external: {self.name}"

    def exec(self, operation_name: str, args: list):
        """Executes the given operation on a list of arguments.
        
        This function may or may not return a value. The case for returning
        a value corresponds to call `ret := op(args)`, while the case for
        not returning a value corresponds to call `op(args)`.

        """
        raise NotImplementedError(f"exec not implemented for {self.name}")


class DefaultSimulatorExternal(SimulatorExternal):
    """The default external supporting no functions."""
    def __init__(self):
        self.name = "default"
...
class Callstack:
    ...
    def getinfo(self, hp: hcsp.HCSP, procs: dict[str, hcsp.Procedure]):
        self.renewinnerpos()
        callstack_info = {
            'innerpos': [],
            'procedure': []
        ...
        while index >= 0:
            ...
            elif not isinstance(get_pos(hp, self.callstack[-1].pos, procs), hcsp.Wait):
...
def get_pos(hp: hcsp.HCSP, pos, procs: dict[str, hcsp.Procedure] = None) -> hcsp.HCSP:
...
def parse_pos(hp: hcsp.HCSP, pos: str):
    ...
    if len(pos) > 0 and pos[-1].startswith('w'):
        ...
        assert isinstance(get_pos(hp, pos), hcsp.Wait)
    else:
        pos = tuple(int(p) for p in pos)

    return pos

def disp_of_callstack(info: "SimInfo"):
...
class SimInfo:
    ...
    def __init__(self, name: str, hp: hcsp.HCSP, *,
                 outputs: Iterable[hcsp.HCSPOutput] = tuple(),
    ...
    def get_ode_delay(self, hp: hcsp.HCSP):
        """Obtain the delay needed for the given ODE, starting at the
        given state.
        
        """
        assert isinstance(hp, (hcsp.ODE, hcsp.ODE_Comm))
    ...
    def exec_assign(self, lname: Expr, val, hp: hcsp.HCSP):
    ...
    def exec_step(self, external: SimulatorExternal = None):
        ...
        elif isinstance(cur_hp, hcsp.External):
            args = [self.eval_expr(arg) for arg in cur_hp.args]
            res = external.exec(cur_hp.func_name, args)
            if cur_hp.var_name is not None:
                self.exec_assign(cur_hp.var_name, res, cur_hp)
            self.step_pos()
            self.reason = None

        elif isinstance(cur_hp, hcsp.Assert):
        ...
        elif isinstance(cur_hp, hcsp.Test):
        ...
        elif isinstance(cur_hp, hcsp.Log):
        ...
        elif isinstance(cur_hp, hcsp.Var):
            # Return to body of recursion
            for i in range(len(self.callstack.top_pos())):
                hp = get_pos(self.hp, self.callstack.top_pos()[:i], self.procedures)
        ...
        elif isinstance(cur_hp, hcsp.InputChannel):
            # Waiting for input
            self.reason = {"comm": [(self.eval_channel(cur_hp.ch_name), "?")]}

        elif isinstance(cur_hp, hcsp.OutputChannel):
            # Waiting for someone to receive output
            self.reason = {"comm": [(self.eval_channel(cur_hp.ch_name), "!")]}

        elif isinstance(cur_hp, hcsp.Wait):
        ...
        elif isinstance(cur_hp, hcsp.ODE_Comm):
            # Run ODE until one of the communication events (or the boundary)
            comms = []
            for io_comm, _ in cur_hp.io_comms:
                if isinstance(io_comm, hcsp.InputChannel):
                    comms.append((self.eval_channel(io_comm.ch_name), "?"))
                elif isinstance(io_comm, hcsp.OutputChannel):
                    comms.append((self.eval_channel(io_comm.ch_name), "!"))
                else:
                    raise NotImplementedError
        ...
        elif isinstance(cur_hp, hcsp.SelectComm):
            # Waiting for one of the input/outputs
            comms = []
            for comm_hp, _ in cur_hp.io_comms:
                if isinstance(comm_hp, hcsp.InputChannel):
                    comms.append((self.eval_channel(comm_hp.ch_name), "?"))
                elif isinstance(comm_hp, hcsp.OutputChannel):
                    comms.append((self.eval_channel(comm_hp.ch_name), "!"))
                else:
                    raise NotImplementedError
            self.reason = {"comm": comms}
    ...
    def exec_input_comm(self, ch_name, x, inst=None):
        ...
        if isinstance(cur_hp, hcsp.InputChannel):
            ...
            for i, (comm_hp, out_hp) in enumerate(cur_hp.io_comms):
                if isinstance(comm_hp, hcsp.InputChannel) and self.eval_channel(comm_hp.ch_name) == ch_name:
    ...
    def exec_output_comm(self, ch_name, inst=None):
        ...
        if isinstance(cur_hp, hcsp.OutputChannel):
            assert self.eval_channel(cur_hp.ch_name) == ch_name
            self.step_pos()
            ...
            for i, (comm_hp, out_hp) in enumerate(cur_hp.io_comms):
                if isinstance(comm_hp, hcsp.OutputChannel) and self.eval_channel(comm_hp.ch_name) == ch_name:
    ...
    def exec_delay(self, delay, *, time_series=None):
        ...
        cur_hp = get_pos(self.hp, self.callstack.top_pos(), self.procedures)
        if isinstance(cur_hp, (hcsp.InputChannel, hcsp.OutputChannel, hcsp.SelectComm)):
            pass
        ...
        elif isinstance(cur_hp, (hcsp.ODE_Comm, hcsp.ODE)):
            finish_ode = False
...
def extract_event(infos: list[SimInfo]):
...
def exec_parallel(infos: list[SimInfo], *, num_io_events=None, num_steps=3000, num_show=None,
                  ...
                  external: SimulatorExternal = DefaultSimulatorExternal()):
