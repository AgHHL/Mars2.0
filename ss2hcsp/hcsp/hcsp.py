...
from typing import Dict, Iterable, List, Iterator, Optional, Tuple, Union, Set

from ss2hcsp.hcsp import expr
from ss2hcsp.hcsp.expr import Expr, AVar, AConst, BConst, Expr, true_expr, false_expr, \
    RelExpr, Expr, FunExpr
from ss2hcsp.hcsp import assertion
from ss2hcsp.hcsp.assertion import OrdinaryAssertion
...
class Channel:
    ...
    def __eq__(self, other):
        return isinstance(other, Channel) and self.name == other.name and self.args == other.args
    ...
    def subst(self, inst: dict[str, Union[Expr, "Channel"]]) -> "Channel":
...
def replace_function(e: FunExpr, funcs: dict[str, Function]):
    """Replace a FunExpr by the expr of its corresponding Function object.

    For example, for input expression `bar(2 * x)`, with `funcs` is
        `{'bar': Function('bar', ['x'], 'x + 1')}`,
    return expression `2 * x + 1`.
...
def subst_all_funcs(e: Expr, funcs: dict[str, Function]):
    """Substitute all defined FunExpr in e. Return an expression without
    defined FunExprs.

    For example,
        funcs = {'f': hcsp.Function('f', ['x'], "x + 1"),
                 'g': hcsp.Function('g', ['x'], "f(x) + 2")},
        For expression g(y) * 2, return (y + 1 + 2) * 2
    ...
    def rec(e: Expr):
        if isinstance(e, (AConst, BConst, AVar)):
            ...
            return expr.OpExpr(e.op, *[rec(expr) for expr in e.exprs])
        elif isinstance(e, RelExpr):
            return RelExpr(e.op, rec(e.expr1), rec(e.expr2))
        elif isinstance(e, expr.LogicExpr):
            return expr.LogicExpr(e.op, *[rec(expr) for expr in e.exprs])
        elif isinstance(e, expr.ExistsExpr):
            return expr.ExistsExpr(e.vars, rec(e.expr))
        elif isinstance(e, expr.ForAllExpr):
            return expr.ForAllExpr(e.vars, rec(e.expr))
        elif isinstance(e, expr.IfExpr):
            return expr.IfExpr(rec(e.cond), rec(e.expr1), rec(e.expr2))
        else:
...
class HCSP:
    ...
    def size(self) -> int:
        ...
        if isinstance(self, (Var, Skip, Wait, Assign, External, Assert, Test, Log,
    ...
    def contain_hp(self, name) -> bool:
        ...
        elif isinstance(self, (Skip, Wait, Assign, External, Assert, Test, Log,
                               InputChannel, OutputChannel)):
            return False
        elif isinstance(self, (Sequence, Parallel)):
        ...
        def rec(hp):
            if isinstance(hp, Var):
                if hp.name not in res:
                    res[hp.name] = 0
                res[hp.name] += 1
            elif isinstance(hp, (Skip, Wait, Assign, External, Assert, Test, Log,
                                 InputChannel, OutputChannel)):
                pass
    ...
    def subst_comm(self, inst: dict[str, Union[Expr , Channel]]):
        ...
        if isinstance(self, (Var, Skip)):
            ...
            return External(self.var_name, self.func_name, [arg.subst(inst) for arg in self.args])
...
class Var(HCSP):
    ...
    def __eq__(self, other):
        return isinstance(other, Var) and self.type == other.type and self.name == other.name
...
class Skip(HCSP):
    ...
    def __eq__(self, other):
        return isinstance(other, Skip) and self.type == other.type
...
class Wait(HCSP):
    ...
    ----------
    delay : Expr
        expression that evaluates to the time delay.

    """
    def __init__(self, delay: Expr, meta=None):
    ...
    def __eq__(self, other):
        return isinstance(other, Wait) and self.type == other.type and self.delay == other.delay
...
class Assign(HCSP):
    ...
    ----------
    var_name : Expr
        variable to be assigned, one of AVar (e.g. x), ArrayIdxExpr (e.g. a[0]),
        and FieldNameExpr (e.g. a.field1).
    expr : Expr
    ...
    def __eq__(self, other):
        return isinstance(other, Assign) and self.type == other.type and \
            self.var_name == other.var_name and self.expr == other.expr
...
class RandomAssign(HCSP):
    ...
    var_name : Expr
        ...
        and FieldNameExpr (e.g. a.field1).
    expr : Expr
        the range value to be randomly choose from.
    ...
    def __eq__(self, other):
        return isinstance(other, RandomAssign) and self.type == other.type and \
            self.var_name == other.var_name and self.expr == other.expr
...
class External(HCSP):
    """Represents call to an external function.
    
    Attributes
    ----------
    var_name: Optional[Expr]
        variable to which return value of the function is assigned.
        None if no return value need to be assigned.
    func_name: str
    ...
    """
    def __init__(self, var_name: Optional[Expr], func_name: str,
                 args: Iterable[Expr], meta=None):
    ...
    def __eq__(self, other):
        return isinstance(other, External) and self.var_name == other.var_name and \
            self.func_name == other.func_name and self.args == other.args

    def __repr__(self):
        return "External(%s, %s, %s)" % (
            self.var_name, self.func_name, repr(self.args))
    
    def __str__(self):
        return "%s := %s(%s);" % (self.var_name, self.func_name,
                                  ", ".join(str(arg) for arg in self.args))

    def __hash__(self):
        return hash(("External", self.var_name, self.func_name, self.args))
    ...
    def get_vars(self):
        ...
        for arg in self.args:
            var_set = var_set.union(arg.get_vars())
        return var_set

    def get_func_names(self):
        res = set()
        for arg in self.args:
            res = res.union(arg.get_fun_names())
        return res

    def get_zero_arity_funcs(self):
        res = set()
        for arg in self.args:
            res = res.union(arg.get_zero_arity_funcs())
        return res
...
class Assert(HCSP):
    def __init__(self, bexpr: Expr, msgs: Iterable[Expr], meta=None):
    ...
    def __eq__(self, other):
        return isinstance(other, Assert) and self.type == other.type and \
            self.bexpr == other.bexpr and self.msgs == other.msgs
...
class Test(HCSP):
    def __init__(self, bexpr: Expr, msgs, meta=None):
    ...
    def __eq__(self, other):
        return isinstance(other, Test) and self.type == other.type and \
            self.bexpr == other.bexpr and self.msgs == other.msgs
...
class Log(HCSP):
    def __init__(self, pattern: Expr, *, exprs: Iterable[Expr] = None, meta=None):
    ...
    def __eq__(self, other):
        return isinstance(other, Log) and self.type == other.type and \
            self.pattern == other.pattern and self.exprs == other.exprs
...
class InputChannel(HCSP):
    ...
    def __eq__(self, other):
        return isinstance(other, InputChannel) and self.type == other.type and \
            self.ch_name == other.ch_name and self.var_name == other.var_name
...
class OutputChannel(HCSP):
    ...
    def __eq__(self, other):
        return isinstance(other, OutputChannel) and self.type == other.type and \
            self.expr == other.expr and self.ch_name == other.ch_name
...
class ODE(HCSP):
    ...
    """
    def __init__(self, eqs: Iterator[tuple[str, Expr]], constraint: Expr, *,
                 out_hp: HCSP = Skip(), meta=None, rule="dw", unsols=[], ghosts=tuple(), inv=tuple()):
        ...
        self.rule = rule
        assert isinstance(unsols, list)
        for unsol in unsols:
            assert isinstance(unsol, str)
        ...
        self.type = "ode"
        self.eqs: tuple[tuple[str, Expr]] = tuple(eqs)
        self.constraint: Expr = constraint
        self.out_hp: HCSP = out_hp
        self.meta = meta
        self.unsols = unsols
        self.ghosts = ghosts
        self.inv = inv
    ...
    def __eq__(self, other):
        return isinstance(other, ODE) and self.eqs == other.eqs and \
               ...
               self.unsols == other.unsols and self.ghosts == other.ghosts and self.inv == other.inv
...
class ODE_Comm(HCSP):
    ...
    def __init__(self, eqs: List[Tuple[str, Expr]], constraint: Expr,
                 io_comms: List[Tuple[HCSP, HCSP]], meta=None, unsols=[], inv=tuple()):
    ...
    def __eq__(self, other):
        return self.type == other.type and self.eqs == other.eqs and \
               self.constraint == other.constraint and self.io_comms == other.io_comms and \
               self.unsols == other.unsols and self.inv == other.inv
...
class ODE_Commn(HCSP):
    """Represents an ODE program that can be interrupted by
    communications, of the form

    {F(s',s) = 0 & B -> c} |> [] (io_i --> Q_i)

    """
    def __init__(self, eqs: List[Tuple[str, Expr]], constraint: Expr, tail: HCSP,
                 io_comms: List[Tuple[HCSP, HCSP]], meta=None, unsols=[], inv=tuple()):
        """Each equation is of the form (var_name, expr). Each element
        of io_comms is of the form (comm_hp, out_hp), where comm_hp
        is a communication process (either InputChannel or OutputChannel).
        ...
        """
        super(ODE_Commn, self).__init__()
        assert isinstance(eqs, list)
        for eq in eqs:
            assert isinstance(eq, tuple) and len(eq) == 2
            assert isinstance(eq[0], str) and isinstance(eq[1], Expr)
        assert isinstance(constraint, Expr)
        assert isinstance(tail, HCSP)
        assert isinstance(io_comms, list)
        for io_comm in io_comms:
            assert isinstance(io_comm, tuple) and len(io_comm) == 2
            assert is_comm_channel(io_comm[0]) and isinstance(io_comm[1], HCSP)
    ...
    def __eq__(self, other):
        return self.type == other.type and self.eqs == other.eqs and \
               self.constraint == other.constraint and self.tail == other.tail and \
               self.io_comms == other.io_comms and self.unsols == other.unsols and self.inv == other.inv

    def __str__(self):
        str_eqs = ", ".join(var_name + "_dot = " + str(expr) for var_name, expr in self.eqs)
        str_io_comms = ", ".join(str(comm_hp)[:-1] + " --> " + str(out_hp) for comm_hp, out_hp in self.io_comms)
        return "{" + str_eqs + " & " + str(self.constraint) + "+>" + str(self.tail) + "} |> [] (" + str_io_comms + ")"

    def __repr__(self):
        str_eqs = ", ".join(var_name + ", " + str(expr) for var_name, expr in self.eqs)
        str_io_comms = ", ".join(str(comm_hp)[:-1] + ", " + str(out_hp) for comm_hp, out_hp in self.io_comms)
        return "ODECommn(%s, %s, %s, %s)" % (str_eqs, str(self.constraint), str(self.tail), str_io_comms)

    def __hash__(self):
        return hash(("ODE_Commn", self.eqs, self.constraint, self.tail, self.io_comms))
    ...
    def get_vars(self):
        ...
        for variable, expression in self.eqs:
            ...
            var_set.update(expression.get_vars())
        var_set.update(self.constraint.get_vars())
        var_set.update(self.tail.get_vars())
        for ch, out_hp in self.io_comms:
            var_set.update(ch.get_vars())
            var_set.update(out_hp.get_vars())
        return var_set
    ...
    def get_fun_names(self):
        ...
        for _, expression in self.eqs:
            fun_set.update(expression.get_fun_names())
        fun_set.update(self.constraint.get_fun_names())
        fun_set.update(self.tail.get_fun_names())
        for ch, out_hp in self.io_comms:
            fun_set.update(ch.get_fun_names())
            fun_set.update(out_hp.get_fun_names())
        return fun_set
    ...
    def get_zero_arity_funcs(self):
        ...
        for _, expression in self.eqs:
            fun_set.update(expression.get_zero_arity_funcs())
        fun_set.update(self.constraint.get_zero_arity_funcs())
        fun_set.update(self.tail.get_zero_arity_funcs())
        for ch, out_hp in self.io_comms:
            fun_set.update(ch.get_zero_arity_funcs())
            fun_set.update(out_hp.get_zero_arity_funcs())
        return fun_set
    ...
    def get_chs(self):
        ch_set = set()
        ch_set.update(self.tail.get_chs())
        for ch, out_hp in self.io_comms:
            ch_set.update(ch.get_chs())
            ch_set.update(out_hp.get_chs())
        return ch_set
...
class Pnseqhp(HCSP):
    ...
    def __init__(self, pn, hp, meta=None):
        super(Pnseqhp, self).__init__()
        self.type = 'pnseq'
        assert isinstance(hp, HCSP)
    ...
    def __eq__(self, other):
        return self.type == other.type and self.hp == other.hp and self.pn == other.pn 

    def __repr__(self):
        return "PnSeq{%s: %s}" % (str(self.pn), repr(self.hp))

    def __str__(self):
        return "%s: %s" % (str(self.pn), repr(self.hp))

    # def __hash__(self):
    #     return hash(("PnSeq", self.pn, self.hp))
    ...
    def get_vars(self):
        return set(self.pn+str(n) for n in self.hp.get_vars())
...
class PParallel(HCSP):
    def __init__(self, hp1, hp2, chs, meta=None):
        """hps is a list of hybrid programs."""
        super(PParallel, self).__init__()
        self.type = "parallel"
        assert isinstance(hp1, HCSP)
        assert isinstance(hp2, HCSP)
    ...
    def __eq__(self, other):
        return self.type == other.type and self.hp1 == other.hp1 and self.hp2 == other.hp2 and self.chs == other.chs

    def __repr__(self):
        chs_str = "{%s}" % (','.join(str(n) for n in self.chs))
        return "Parallel(%s, %s, %s)" % (repr(self.hp1), repr(self.hp2), chs_str)

    def __str__(self):
        chs_str = "{%s}" % (','.join(str(n) for n in self.chs))
        return "(" + str(self.hp1) + "|" + chs_str + "|" + str(self.hp2) + ")"
    ...
    def get_vars(self):
        var_set = set()
        var_set.update(self.hp1.get_vars())
        var_set.update(self.hp2.get_vars())
        return var_set

    def get_fun_names(self):
        fun_set = set()
        fun_set.update(self.hp1.get_fun_names())
        fun_set.update(self.hp2.get_fun_names())
        return fun_set

    def get_zero_arity_funcs(self):
        fun_set = set()
        fun_set.update(self.hp1.get_zero_arity_funcs())
        fun_set.update(self.hp2.get_zero_arity_funcs())
        return fun_set


    def get_chs(self):
        ch_set = set()
        ch_set.update(self.hp1.get_chs())
        ch_set.update(self.hp2.get_chs())
        return ch_set
...
def get_comm_chs(hp: HCSP):
    ...
    def rec(_hp: HCSP):
        if isinstance(_hp, InputChannel):
            collect.append((_hp.ch_name, '?'))
        elif isinstance(_hp, OutputChannel):
            collect.append((_hp.ch_name, '!'))
...
class HoareTriple:
    """Represents a Hoare triple, containing a program with pre- and
    postconditions, as well as constant and function declarations.
    
    """
    def __init__(self, pre: Iterable[OrdinaryAssertion],
                 hp: HCSP,
                 post: Iterable[OrdinaryAssertion],
                 constants=Constants(),
        ...
        self.pre: list[OrdinaryAssertion] = list(pre)
        self.post: list[OrdinaryAssertion] = list(post)
        self.hp: HCSP = hp
...
class HoareTriplePar:
    """A program with pre- and post-conditions"""
    def __init__(self, pre: Iterable[OrdinaryAssertion], 
                 hp: HCSP,
                 post: Iterable[OrdinaryAssertion], 
                 postr: Iterable[OrdinaryAssertion], 
                 functions: dict[str, Function] = None, 
                 loopinv={}, odeass=[], meta=None):
        self.pre = list(pre)
        self.post = list(post)
        self.postr = list(postr)
        self.hp = hp
        if functions is None:
            functions = dict()
        self.functions = functions
        self.loopinv = loopinv
        self.odeass = odeass
        self.meta = meta
...
class Procedure:
    ...
    def __eq__(self, other):
        return isinstance(other, Procedure) and self.name == other.name and \
            self.hp == other.hp
...
class HCSPInfo:
    ...
    This represents an HCSP process with possible list of outputs
    and procedures.
    
    """
    def __init__(self, name: str, hp: HCSP, *, outputs: Iterable[HCSPOutput] = tuple(),
    ...
    def __eq__(self, other):
        return isinstance(other, HCSPInfo) and self.name == other.name and \
            self.hp == other.hp


def subst_comm_all(hp: HCSP, inst: dict[str, Union[Expr , Channel]]):
...
def subst_all(e: Expr, inst: dict[str, Union[Expr , Channel]]):
...
def get_concrete_chs(infos: list[HCSPInfo]) -> dict[str, set[tuple[Expr]]]:
    ...
    def rec(hp):
        ...
        elif isinstance(hp, (Var, Skip, Wait, Assign, External, Assert, Test, Log)):
            pass
        elif isinstance(hp, Loop):
            rec(hp.hp)
        elif isinstance(hp, Sequence):
...
def reduce_procedures(hp: HCSP, procs: dict[str, HCSP],
                      strict_protect: Optional[set[str]] = None):
    ...
    hp : HCSP
        input process.
    procs : dict[str, HCSP]
        mapping from procedure name to procedure body.
    strict_protect : set[str]
        this set of procedures will never be inlined.
    ...
    dep_relation: dict[str, set[HCSP]] = dict()
