...
def pprint_lines(hp: hcsp.HCSP, *, max_line=None, record_pos=False):
    ...
    def new_line(indent: int):
        """Start a new line with given indentation"""
        lines.append(" " * indent)

    def add_str(s: str):
        """Add string to the last line"""
        lines[-1] += s

    def start_pos(pos: tuple[int]):
        if record_pos:
            pos = 'p' + ','.join(str(p) for p in pos)
            mapping[pos] = {'start_x': len(lines)-1, 'start_y': len(lines[-1])}

    def end_pos(pos: tuple[int]):
        if record_pos:
            pos = 'p' + ','.join(str(p) for p in pos)
            mapping[pos].update({'end_x': len(lines)-1, 'end_y': len(lines[-1])})

    def rec(hp: hcsp.HCSP, indent: int, pos: tuple[int]):
